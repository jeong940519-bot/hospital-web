...
function importFontFile(file, onDone, quiet){
  if(!file) return;
  ...
  r.onload=()=>{
    ...
    if(!b64){ if(!quiet) toast('폰트 파일을 읽지 못했습니다'); if(onDone) onDone(null); return; }
    // 저장
    ...
    if(!quiet) toast(`"${family}" 폰트 추가됨`);
    if(onDone) onDone(family);
  };
  r.readAsDataURL(file);
}
// 여러 폰트 파일을 한 번에 가져오기 (다중 선택 / 드래그앤드롭) — 끝나면 요약 토스트 1번
function importFontFiles(files, onDone){
  const list=[...(files||[])].filter(f=>/\.(ttf|otf|woff2?)$/i.test(f.name)||/font/i.test(f.type||''));
  if(!list.length){ toast('폰트 파일(.ttf / .otf / .woff)을 넣어주세요'); return; }
  let done=0, ok=0, last=null;
  list.forEach(f=>importFontFile(f, fam=>{
    done++; if(fam){ ok++; last=fam; }
    if(done===list.length){ toast(ok>1?`폰트 ${ok}개 추가됨`:(last?`"${last}" 폰트 추가됨`:'폰트를 읽지 못했습니다')); if(onDone) onDone(last); }
  }, true));
}
...
export { project, selId, selIds, zoom, page, canvas, save, snapshot, renderCanvas, renderProps, toast, hamburgerRootPages, uid, FONTS, _fontOpts, _clearSel, _resetCpTarget, editorDevice };
...
function _styleOf(e){
  ...
  if(e.type==='shape'){
    const s=pick(['fill','grad','gradient','borderColor','borderW','radius','shadow','shape','opacity','clip']);
    // 도형 안 글자 서식 — 렌더 시 ||기본값이 적용되는 필드라 실제 저장값이 없어도(한 번도 안 건드렸어도) 표시되는 값 그대로 캡처
    s.stFont=e.stFont||'Noto Sans KR';
    s.stSize=e.stSize||28;
    s.stWeight=e.stWeight||700;
    s.stColor=e.stColor||'#ffffff';
    s.stAlign=e.stAlign||'center';
    s.stValign=e.stValign||'middle';
    s.stItalic=!!e.stItalic;
    s.stUnderline=!!e.stUnderline;
    s.stLineHeight=(e.stLineHeight!=null?e.stLineHeight:1.25);
    s.stLetterSpacing=e.stLetterSpacing||0;
    return s;
  }
...
function addText(){
  ...
  _tabTagNew(e); page().elements.push(e); selId=e.id; afterMutate();
}
function addImage(){
...
function addShape(kind){
  ...
  _tabTagNew(e); page().elements.push(e); selId=e.id; selIds=new Set([e.id]); afterMutate();
  // 최근 사용한 도형 기록
...
// 📑 탭 섹션 한 번에 생성 — 버튼 N개 + 내용 패널 N개를 하나의 그룹(group)·번호(idx 0..N-1)로 올바르게 배선
// ── 탭 섹션 요소 생성기(공용) — 수동 생성·AI 초안 펼치기 양쪽에서 사용 ──
// opts: {x,y,w,btnH,panelH,activeColor,idleColor, tabs:[{label, body?:[패널 기준 상대좌표 요소들]}]}
// 반환: { els:[...], bottom:Y } — 저수준 tab-trigger/tab-content/twin/group 배선을 정확히 만든다.
function buildTabSectionEls(opts){
  opts=opts||{};
  const G=opts.group||('tabs_'+uid().slice(0,6));
  let tabs=Array.isArray(opts.tabs)&&opts.tabs.length?opts.tabs:[{label:'탭 1'},{label:'탭 2'},{label:'탭 3'}];
  tabs=tabs.slice(0,8); const n=Math.max(2,tabs.length);
  const pw=page().w||PAGE_W;
  const blockW=Math.round(Math.max(280,Math.min(opts.w||Math.min(1100,pw-160), pw)));
  const bx=Math.round(opts.x!=null?opts.x:Math.max(0,(pw-blockW)/2)), by=Math.round(opts.y!=null?opts.y:0);
  const gap=12, btnH=opts.btnH||58, btnW=Math.round((blockW-(n-1)*gap)/n);
  const cY=by+btnH+44, cH=opts.panelH||440;
  const idle=opts.idleColor||'#eef1f6', active=opts.activeColor||'#3b82f6';
  const news=[];
  for(let i=0;i<n;i++){
    const x=Math.round(bx+i*(btnW+gap)), twB=G+'_b'+i, twC=G+'_c'; // 쌍둥이: 버튼 슬롯(선택 전+후) / 내용 슬롯
    const label=(tabs[i]&&tabs[i].label!=null?String(tabs[i].label):'탭 '+(i+1));
    // ① 선택 전(기본) 버튼 — 글자가 들어간 도형 하나(별도 텍스트박스 없음). 항상 보이는 트리거
    news.push({id:uid(),type:'shape',x,y:by,w:btnW,h:btnH,rot:0,shape:'rrect',fill:idle,radius:10,borderW:0,borderColor:'#333333',stext:label,stColor:'#3b4658',stSize:18,stWeight:700,stAlign:'center',stValign:'middle',twin:twB,fx:{type:'tab-trigger',group:G,idx:i}});
    // ② 선택 후 버튼 — 같은 자리에 겹침. 그 탭이 선택됐을 때만 위에 나타남
    news.push({id:uid(),type:'shape',x,y:by,w:btnW,h:btnH,rot:0,shape:'rrect',fill:active,radius:10,borderW:0,borderColor:'#333333',stext:label,stColor:'#ffffff',stSize:18,stWeight:800,stAlign:'center',stValign:'middle',twin:twB,fx:{type:'tab-content',group:G,idx:i}});
    // ③ 내용 — AI가 body를 주면 그 요소들을(패널 기준 상대좌표→절대좌표), 없으면 자리표시 패널
    const body=Array.isArray(tabs[i]&&tabs[i].body)?tabs[i].body.filter(b=>b&&b.type&&b.type!=='tabsection'):null;
    if(body&&body.length){
      // 패널 배경(없으면 깔아줌 — 첫 body가 폭 넓은 shape면 그것을 배경으로 인정)
      const hasBg=body.some(b=>b.type==='shape'&&(b.x||0)<=8&&(b.w||0)>=blockW*0.8);
      if(!hasBg) news.push({id:uid(),type:'shape',x:bx,y:cY,w:blockW,h:cH,rot:0,shape:'rrect',fill:'#f6f7fb',radius:14,borderW:1,borderColor:'#e3e7f0',twin:twC,fx:{type:'tab-content',group:G,idx:i}});
      body.forEach(b=>{
        const o=Object.assign({},b); o.id=uid(); o.rot=o.rot||0;
        o.x=Math.round(bx+(o.x||0)); o.y=Math.round(cY+(o.y||0));
        o.fx={type:'tab-content',group:G,idx:i};
        news.push(o);
      });
    } else {
      news.push({id:uid(),type:'shape',x:bx,y:cY,w:blockW,h:cH,rot:0,shape:'rrect',fill:'#f6f7fb',radius:14,borderW:1,borderColor:'#e3e7f0',stext:label+' 내용 — 「이펙트 ▸ 📑 탭별 편집」에서 이 탭을 골라 채우세요',stColor:'#9aa3c0',stSize:16,stWeight:500,stAlign:'center',stValign:'middle',twin:twC,fx:{type:'tab-content',group:G,idx:i}});
    }
  }
  return { els:news, bottom: cY+cH };
}
function makeTabSection(){
  const raw=prompt('탭 개수를 입력하세요 (2~8)','3'); if(raw==null) return;
  const n=Math.max(2,Math.min(8,parseInt(raw,10)||0)); if(n<2){ toast('탭은 2개 이상이어야 합니다'); return; }
  const blockW=Math.min(1100,(page().w||PAGE_W)-160);
  const c=getViewCenter(blockW,560);
  const tabs=[]; for(let i=0;i<n;i++) tabs.push({label:'탭 '+(i+1)});
  const { els }=buildTabSectionEls({x:c.x,y:c.y,w:blockW,tabs});
  page().elements.push(...els);
  selId=null; selIds=new Set(els.filter(e=>e.fx&&e.fx.type==='tab-trigger').map(e=>e.id));
  afterMutate();
  toast(`탭 섹션 생성 ✨ (${n}개) — 탭 버튼 선택 후 「이펙트 ▸ 📑 탭별 편집」으로 한 탭씩 채우세요`);
}
// AI 초안의 고수준 컴포넌트 노드(tabsection 등)를 저수준 요소로 펼친다. (검증·정규화 전에 호출)
function expandAiComponents(elements){
  const out=[];
  for(const e of (Array.isArray(elements)?elements:[])){
    if(e&&e.type==='tabsection'){
      try{
        const r=buildTabSectionEls({ x:e.x, y:e.y, w:e.w, btnH:e.btnH, panelH:e.panelH,
          activeColor:e.activeColor||e.color, idleColor:e.idleColor, tabs:e.tabs });
        out.push(...r.els);
      }catch(_){ /* 잘못된 노드는 건너뜀 */ }
    } else if(e&&e.type){ out.push(e); }
  }
  return out;
}
...
let _iconResults=[];
// 한글 아이콘 키워드 → 영어 (자주 쓰는 것). 없으면 부분매칭 → AI 번역 폴백
const ICON_KO={
  /* ── 기본 UI ── */
  '전화':'phone','통화':'call','이메일':'email','메일':'mail','위치':'location','주소':'map pin','지도':'map',
  '시계':'clock','시간':'time','달력':'calendar','일정':'calendar','예약':'booking','집':'home','홈':'home',
  '사람':'user','사용자':'user','고객':'user','팀':'group','설정':'settings','톱니':'gear','검색':'search','돋보기':'search',
  '장바구니':'cart','카트':'cart','돈':'money','결제':'payment','카드':'credit card','별':'star','하트':'heart',
  '좋아요':'thumbs up','알림':'bell','종':'bell','메뉴':'menu','닫기':'close','화살표':'arrow',
  '다운로드':'download','업로드':'upload','공유':'share','링크':'link','사진':'image','이미지':'image','카메라':'camera',
  '비디오':'video','영상':'video','음악':'music','파일':'file','폴더':'folder','문서':'document',
  '편집':'edit','연필':'pencil','삭제':'trash','휴지통':'trash','저장':'save','잠금':'lock','자물쇠':'lock',
  '보안':'shield','방패':'shield','체크':'check','확인':'check','경고':'warning','정보':'info','질문':'question',
  '추가':'plus','더하기':'plus','빼기':'minus','태양':'sun','해':'sun','달':'moon','날씨':'weather',
  '비':'rain','눈':'snow','불':'fire','물':'water','나무':'tree','꽃':'flower',
  '자동차':'car','차':'car','버스':'bus','기차':'train','비행기':'airplane','자전거':'bicycle',
  '건물':'building','회사':'company','가게':'store','상점':'store','선물':'gift','트로피':'trophy',
  '메달':'medal','상':'award','책':'book','학교':'school','졸업':'graduation','계산기':'calculator',
  '차트':'chart','그래프':'chart','통계':'analytics','지구':'globe','세계':'globe','깃발':'flag','핀':'pin',
  '태그':'tag','북마크':'bookmark','댓글':'comment','채팅':'chat','메시지':'message','대화':'chat',
  '와이파이':'wifi','배터리':'battery','전원':'power','새로고침':'refresh','재생':'play','정지':'stop',
  '일시정지':'pause','볼륨':'volume','소리':'sound','마이크':'microphone','헤드폰':'headphone',
  '프린터':'printer','인쇄':'print','컴퓨터':'computer','노트북':'laptop','핸드폰':'smartphone',
  '휴대폰':'smartphone','태블릿':'tablet','키보드':'keyboard','마우스':'mouse','로봇':'robot',
  '손':'hand','눈알':'eye','운동':'exercise','헬스':'fitness','요가':'yoga',
  '음식':'food','밥':'food','커피':'coffee','케이크':'cake','과일':'fruit','채소':'vegetable',
  '꽃집':'flower','반려동물':'pet','강아지':'dog','고양이':'cat',
  '화살표오른쪽':'arrow right','화살표왼쪽':'arrow left','위':'arrow up','아래':'arrow down',
  '잠깐':'pause','취소':'close','뒤로':'arrow left','앞으로':'arrow right',
  '리뷰':'review','평점':'rating star','후기':'testimonial','문의':'inquiry','상담':'consultation',
  '온라인상담':'video chat','화상상담':'video call','인증':'verified badge','공지':'megaphone',
  '뉴스':'newspaper','이벤트':'event','쿠폰':'coupon','할인':'sale tag','포인트':'coin',
  '랭킹':'ranking','순위':'ranking','숫자':'number','목록':'list','표':'table',
  '그리드':'grid','레이아웃':'layout','사이드바':'sidebar','팝업':'popup','모달':'window',
  '지갑':'wallet','영수증':'receipt','송금':'transfer','환불':'refund',
  /* ── 병원·의료 기관 ── */
  '병원':'hospital','의원':'clinic','한의원':'chinese medicine','치과':'tooth','안경원':'eyeglasses',
  '한방':'acupuncture','요양원':'nursing home','정신병원':'psychiatry','보건소':'health center',
  '의료센터':'medical center','종합병원':'hospital','대학병원':'university hospital',
  /* ── 의료진 ── */
  '의사':'doctor','간호사':'nurse','원장':'doctor director','교수':'professor','전공의':'resident doctor',
  '인턴':'intern','간호조무사':'nurse assistant','물리치료사':'physical therapist',
  '영양사':'dietitian','약사':'pharmacist','사회복지사':'social worker','원무팀':'office worker',
  '의료진':'medical team','스태프':'staff','직원':'employee',
  /* ── 진료과 ── */
  '내과':'stethoscope','외과':'scalpel','정형외과':'orthopedic','신경과':'brain neurology',
  '신경외과':'neurosurgery','흉부외과':'thoracic heart','심장내과':'cardiology heart',
  '혈관외과':'vein','성형외과':'plastic surgery face','피부과':'skin dermatology',
  '안과':'eye ophthalmology','이비인후과':'ear nose throat','비뇨기과':'kidney urology',
  '산부인과':'obstetrics gynecology','소아과':'pediatric baby','정신건강의학과':'brain mental health',
  '재활의학과':'rehabilitation wheelchair','마취통증의학과':'anesthesia needle',
  '응급의학과':'emergency','가정의학과':'family doctor','영상의학과':'xray radiology',
  '진단검사의학과':'laboratory test tube','종양내과':'cancer oncology',
  '방사선종양학과':'radiation','핵의학과':'nuclear','류마티스내과':'joint arthritis',
  '소화기내과':'stomach digestive','호흡기내과':'lung respiratory','내분비내과':'hormone thyroid',
  '신장내과':'kidney','감염내과':'virus infection','혈액내과':'blood hematology',
  '척추센터':'spine','관절센터':'joint','통증클리닉':'pain clinic',
  /* ── 신체 부위 ── */
  '머리':'head','얼굴':'face','뇌':'brain','눈':'eye','귀':'ear','코':'nose','입':'mouth',
  '치아':'tooth','혀':'tongue','목':'neck','목구멍':'throat','어깨':'shoulder','팔':'arm',
  '팔꿈치':'elbow','손목':'wrist','손가락':'finger','가슴':'chest','폐':'lung',
  '심장':'heart','위장':'stomach','간':'liver','신장':'kidney','방광':'bladder',
  '대장':'colon','소장':'intestine','췌장':'pancreas','담낭':'gallbladder','비장':'spleen',
  '자궁':'uterus','난소':'ovary','전립선':'prostate','척추':'spine','허리':'back pain',
  '골반':'pelvis','엉덩이':'hip','허벅지':'thigh','무릎':'knee','종아리':'calf',
  '발목':'ankle','발':'foot','발가락':'toe','피부':'skin','근육':'muscle',
  '뼈':'bone','관절':'joint','혈관':'vein','신경':'nerve','갑상선':'thyroid','림프':'lymph',
  /* ── 증상 ── */
  '통증':'pain','아픔':'pain','두통':'headache','편두통':'migraine','복통':'stomachache',
  '요통':'back pain','흉통':'chest pain','관절통':'joint pain','근육통':'muscle pain',
  '어지럼증':'dizziness','구역질':'nausea','구토':'vomiting','발열':'fever','기침':'cough',
  '호흡곤란':'breathing','피로':'fatigue','불면증':'insomnia','우울':'depression sad',
  '불안':'anxiety','소화불량':'indigestion','설사':'diarrhea','변비':'constipation',
  '부종':'swelling','발진':'rash','가려움':'itch','출혈':'bleeding','골절':'fracture',
  '탈구':'dislocation','타박상':'bruise','염좌':'sprain','화상':'burn','절상':'wound cut',
  '감염':'infection','콧물':'runny nose','재채기':'sneeze','오한':'chills','식은땀':'sweating',
  '식욕부진':'appetite loss','체중감소':'weight loss','부종':'edema swelling','황달':'jaundice',
  '혈뇨':'blood urine','혈변':'blood stool','시력저하':'vision loss','이명':'tinnitus',
  '난청':'hearing loss','언어장애':'speech disorder','마비':'paralysis','경련':'seizure',
  /* ── 질환·질병 ── */
  '당뇨':'diabetes','고혈압':'hypertension','저혈압':'low blood pressure','심장병':'heart disease',
  '뇌졸중':'stroke brain','뇌경색':'brain infarction','뇌출혈':'brain hemorrhage',
  '암':'cancer','폐암':'lung cancer','위암':'stomach cancer','간암':'liver cancer',
  '유방암':'breast cancer','대장암':'colon cancer','전립선암':'prostate cancer',
  '갑상선암':'thyroid cancer','자궁암':'uterus cancer','백혈병':'leukemia',
  '천식':'asthma','폐렴':'pneumonia','결핵':'tuberculosis','기관지염':'bronchitis',
  '간염':'hepatitis','위염':'gastritis','대장염':'colitis','위궤양':'stomach ulcer',
  '관절염':'arthritis','골다공증':'osteoporosis','디스크':'spine disc','척추측만증':'scoliosis',
  '치매':'dementia','파킨슨':'brain parkinson','뇌전증':'epilepsy seizure',
  '우울증':'depression','공황장애':'panic disorder','불안장애':'anxiety disorder',
  '비만':'obesity','신장병':'kidney disease','요로감염':'urinary infection','방광염':'bladder',
  '알레르기':'allergy','아토피':'atopy skin','건선':'psoriasis','습진':'eczema',
  '백내장':'cataract eye','녹내장':'glaucoma','황반변성':'macular degeneration',
  '중이염':'ear infection','비염':'rhinitis nose','편도염':'tonsillitis','인후염':'pharyngitis',
  '빈혈':'anemia blood','고지혈증':'cholesterol','통풍':'gout','루프스':'lupus',
  '크론병':'crohn bowel','과민성대장':'irritable bowel','역류성식도염':'acid reflux',
  '대사증후군':'metabolic syndrome','수면무호흡':'sleep apnea','화병':'stress anger',
  /* ── 검사·처치·시술 ── */
  '주사':'injection syringe','약':'pill','약처방':'prescription','처방전':'prescription document',
  '수술':'surgery','수혈':'blood transfusion','링거':'iv drip','산소마스크':'oxygen mask',
  '청진기':'stethoscope','혈압계':'blood pressure monitor','체온계':'thermometer',
  '혈당계':'glucose meter','엑스레이':'xray','엠알아이':'mri scan','씨티':'ct scan',
  '내시경':'endoscope','초음파':'ultrasound','심전도':'electrocardiogram','뇌파':'brain wave',
  '혈액검사':'blood test','소변검사':'urine test','조직검사':'biopsy','유전자검사':'dna test',
  '건강검진':'health checkup','독감검사':'flu test','코로나검사':'covid test',
  '재활치료':'rehabilitation therapy','물리치료':'physical therapy','도수치료':'manual therapy',
  '충격파치료':'shockwave therapy','레이저치료':'laser therapy','방사선치료':'radiation therapy',
  '항암치료':'chemotherapy','투석':'dialysis','이식':'transplant','성형':'plastic surgery',
  '지방흡입':'liposuction','보톡스':'botox injection','필러':'filler','레이저':'laser',
  '임플란트':'dental implant','교정':'orthodontics braces','스케일링':'teeth cleaning',
  '라식':'lasik eye','라섹':'lasek eye','시술':'procedure','봉합':'suture stitch',
  '석고붕대':'cast plaster','보조기':'brace support','목발':'crutch','휠체어':'wheelchair',
  '마취':'anesthesia','수면마취':'sedation','전신마취':'general anesthesia',
  '백신':'vaccine','예방접종':'vaccination','독감예방':'flu shot',
  /* ── 병원 시설·업무 ── */
  '접수':'reception desk','대기실':'waiting room','진료실':'clinic room','입원실':'hospital room',
  '수술실':'operating room','응급실':'emergency room','중환자실':'icu intensive care',
  '분만실':'delivery room','신생아실':'newborn baby','영안실':'mortuary',
  '입원':'hospital bed','퇴원':'discharge exit','전원':'transfer ambulance',
  '진단서':'medical certificate','소견서':'medical document','진단':'diagnosis',
  '처방':'prescription','약국':'pharmacy','의료보험':'health insurance',
  '실손보험':'insurance document','보험청구':'insurance claim','건강보험':'insurance card',
  '진료비':'payment medical','영수증':'receipt','주차':'parking','엘리베이터':'elevator',
  '장례식장':'funeral','요양':'nursing care','호스피스':'hospice care',
  /* ── 위생·방역 ── */
  '마스크':'face mask','손소독':'hand sanitizer','소독':'disinfect','방역':'quarantine shield',
  '격리':'quarantine','세균':'bacteria','바이러스':'virus','항균':'antibacterial',
  '위생':'hygiene','청결':'clean','세정':'wash','비누':'soap','장갑':'gloves medical',
  /* ── 건강관리 ── */
  '건강':'health','운동':'exercise','스트레칭':'stretching','걷기':'walking','달리기':'running',
  '수영':'swimming','식단':'diet food','비타민':'vitamin','영양':'nutrition','단백질':'protein',
  '칼로리':'calorie','혈당':'glucose','체중':'weight scale','키':'height','체질량':'bmi',
  '금연':'no smoking','금주':'no alcohol','스트레스':'stress','명상':'meditation','수면':'sleep',
  '임신':'pregnancy','태아':'fetus baby','출산':'birth baby','모유':'breastfeeding',
  '예방':'prevention','면역':'immunity','항체':'antibody'
};
// 의료 관련 키워드 — healthicons 전문 컬렉션 우선 검색 (paint 같은 오염 방지)
const ICON_MEDICAL_RAW=new Set([
  '통증','아픔','두통','편두통','복통','요통','흉통','관절통','근육통',
  '어지럼증','구역질','구토','발열','기침','피로','불면증','골절','출혈','감염',
  '발진','가려움','화상','상처','부상','멍','오한','황달','마비','경련',
  '당뇨','고혈압','저혈압','심장병','뇌졸중','암','폐렴','결핵','기관지염',
  '간염','위염','대장염','관절염','골다공증','디스크','치매','우울증',
  '비만','알레르기','아토피','건선','습진','빈혈','고지혈증','통풍',
  '백내장','녹내장','중이염','비염','편도염','천식','수면무호흡',
  '의사','간호사','병원','의원','청진기','주사','약','수술','치료','재활',
  '혈액','혈압','혈당','체온','산소','백신','예방접종','건강검진',
  '엑스레이','초음파','심전도','내시경','물리치료','도수치료','항암치료',
  '링거','수혈','마취','봉합','처방','처방전','진단','진단서',
  '내과','외과','정형외과','신경외과','신경과','피부과','안과','이비인후과',
  '소아과','산부인과','비뇨기과','재활의학과','응급','구급차','응급실',
  '입원','퇴원','수술실','중환자실','분만실','임신','출산','태아',
  '뇌','폐','심장','간','신장','위장','척추','관절','갑상선','림프',
  '상처','골절','탈구','타박상','염좌','절상'
]);
async function toEnglishQuery(q){
  if(!/[가-힣]/.test(q)) return q;                 // 영어면 그대로
  if(ICON_KO[q]) return ICON_KO[q];                // 정확 매칭
  for(const k in ICON_KO){ if(q.indexOf(k)>=0) return ICON_KO[k]; }  // 부분 매칭
  // AI 번역 폴백 (관리자 로그인 시)
  try{
    const r=await aiProxy({ body:{ model:'claude-sonnet-4-6', max_tokens:30,
      system:'Translate the Korean word to 1-2 English icon search keywords. Output ONLY lowercase English words, nothing else.',
      messages:[{role:'user',content:q}] } });
    const t=(r.data&&r.data.content&&r.data.content[0]&&r.data.content[0].text||'').trim().toLowerCase().replace(/[^a-z0-9 ]/g,'').trim();
    if(t) return t;
  }catch(_){}
  return q;
}
// 원본 SVG(currentColor 유지)에 색·선굵기를 적용해 data URL 생성
function iconSvgToDataUrl(svg, color, stroke){
  let s=svg;
  if(color) s=s.replace(/currentColor/g, color);
  if(stroke!=null){
    if(/stroke-width\s*=/.test(s))
      s=s.replace(/stroke-width\s*=\s*["'][\d.]+["']/g,'stroke-width="'+stroke+'"');
    else
      s=s.replace(/(<svg\b[^>]*?)>/,'$1 stroke-width="'+stroke+'">');
    s=s.replace(/stroke-width\s*:\s*[\d.]+/g,'stroke-width:'+stroke);
  }
  return 'data:image/svg+xml;utf8,'+encodeURIComponent(s);
}
function iconStrokeOf(svg){
  const m=svg.match(/stroke-width\s*=\s*["']([\d.]+)["']/)
         ||svg.match(/stroke-width\s*:\s*([\d.]+)/);
  return m?parseFloat(m[1]):null;
}
function iconStrokeable(svg){
  if(/stroke-width\s*=\s*["'][\d.]/.test(svg)) return true;
  if(/stroke-width\s*:\s*[\d.]/i.test(svg)) return true;
  if(/stroke\s*=\s*["']?currentColor/i.test(svg)) return true;
  if(/stroke\s*:\s*currentColor/i.test(svg)) return true;
  try{
    const doc=new DOMParser().parseFromString(svg,'image/svg+xml');
    return [...doc.querySelectorAll('[stroke]')].some(e=>{ const s=e.getAttribute('stroke'); return s&&s!=='none'; });
  }catch(_){ return false; }
}
function iconMono(svg){ return /currentColor/.test(svg); }
// 색칠 가능한 요소 목록 (7단계 필터링)
function iconColorableEls(doc){
  return [...doc.querySelectorAll('path,circle,rect,ellipse,polygon,polyline,line,text,tspan')].filter(el=>{
    // ① defs·clipPath·mask·symbol 내부 제외
    let p=el.parentElement;
    while(p){ const t=(p.tagName||'').toLowerCase(); if(['defs','clippath','mask','symbol'].includes(t)) return false; p=p.parentElement; }
    ...
    // ② fill=none + stroke 없음(완전 투명)
    if((f==='none'||f==='transparent')&&(!s||s==='none')) return false;
    // ③ opacity=0
    if(el.getAttribute('opacity')==='0') return false;
    // ④ fill-opacity=0 이면서 stroke도 없음
    if(el.getAttribute('fill-opacity')==='0'&&(!s||s==='none')) return false;
    // ⑤ stroke-width=0 이면서 fill=none
    if(sw===0&&(f==='none'||f==='transparent'||!f)) return false;
    // ⑥ display:none / visibility:hidden (인라인 style)
    if(/display\s*:\s*none/i.test(sty)||/visibility\s*:\s*hidden/i.test(sty)) return false;
    // ⑦ 배경 rect 감지 — viewBox 크기 85% 이상 + 흰색·투명 채움
    if((el.tagName||'').toLowerCase()==='rect'){
      const svg=doc.querySelector('svg');
      if(svg){ const vb=(svg.getAttribute('viewBox')||'').trim().split(/[\s,]+/).map(Number);
        if(vb.length>=4){ const vw=vb[2],vh=vb[3],rw=parseFloat(el.getAttribute('width')||0),rh=parseFloat(el.getAttribute('height')||0);
          if(rw>=vw*0.85&&rh>=vh*0.85&&(!f||f==='none'||f==='transparent'||/^(white|#fff|#ffffff)$/i.test(f))) return false; } }
    }
    return true;
  });
}
// 버튼 표시 기준 = 색칠 가능한 요소 수(펜으로 직접 칠해서 선택하는 방식이라 그룹 개념 불필요)
// 버튼 노출 여부 판단용 — 뭉친 path를 조각 단위로 쪼갠 "이후" 개수로 세야 정확함(안 그러면 한 path에 여러 조각이 뭉친 아이콘은 버튼 자체가 숨겨짐)
function iconPartCount(svg){ try{ return iconColorableEls(new DOMParser().parseFromString(_splitIconSubpaths(svg),'image/svg+xml')).length; }catch(_){ return 0; } }
function _iconRootStroke(root){ return root.getAttribute('fill')==='none' || /currentColor/.test(root.getAttribute('stroke')||''); }
function applyPartColor(elDom, rootIsStroke, color){
  const f=elDom.getAttribute('fill'), s=elDom.getAttribute('stroke');
  if(f && f!=='none') elDom.setAttribute('fill', color);
  else if(s && s!=='none') elDom.setAttribute('stroke', color);
  else if(rootIsStroke) elDom.setAttribute('stroke', color);
  else elDom.setAttribute('fill', color);
}
// _iconParts = { 'origColorKey': newColor, ... }  (색상 키 기반 — fill 아이콘 완전 지원)
// origColorKey: 원본 fill/stroke 색을 소문자+6자리 hex 정규화한 문자열
function _normColor(c,base){ const v=(c==='currentColor'?(base||'#333333'):c)||'#333333'; return v.toLowerCase().replace(/^#([0-9a-f])([0-9a-f])([0-9a-f])$/,'#$1$1$2$2$3$3'); }
function _elOrigColor(el2,rs,base){
  const f=el2.getAttribute('fill'), s=el2.getAttribute('stroke');
  if(f&&f!=='none') return _normColor(f,base);
  if(s&&s!=='none') return _normColor(s,base);
  return _normColor(rs?'stroke':base,base);
}
// 부분색 반영해 최종 SVG data URL 생성
// parts 키 형식: "part:N" (요소 인덱스) 또는 레거시 색상 키
// 부분색(parts) + 두께(morphDelta) 반영. 두께는 feMorphology로 전체를 균일하게 굵게/얇게 → fill·stroke 부분이 함께 변함.
// morphDelta: 0=원본, 양수=두껍게(dilate), 음수=얇게(erode)
function renderIconWithParts(rawSvg, baseColor, parts, morphDelta){
  const hasParts=parts&&Object.keys(parts).length;
  const md=(typeof morphDelta==='number'&&Math.abs(morphDelta)>0.05)?morphDelta:0;
  if(!hasParts && !md) return iconSvgToDataUrl(rawSvg, baseColor, null);
  try{
    const doc=new DOMParser().parseFromString(rawSvg,'image/svg+xml');
    const root=doc.querySelector('svg'); if(!root) return iconSvgToDataUrl(rawSvg,baseColor,null);
    const rs=_iconRootStroke(root), base=baseColor||'#333333';
    if(hasParts){
      const els=iconColorableEls(doc);
      els.forEach((el2,i)=>{
        const pk='part:'+i;
        if(parts[pk]!=null){ applyPartColor(el2,rs,parts[pk]); return; }
        const ck=_elOrigColor(el2,rs,base);          // 레거시 색상 키
        if(parts[ck]!=null) applyPartColor(el2,rs,parts[ck]);
      });
    }
    let s=new XMLSerializer().serializeToString(doc);
    if(baseColor) s=s.replace(/currentColor/g, baseColor);
    if(md){
      const op=md>0?'dilate':'erode', r=Math.abs(md).toFixed(2);
      const filt=`<defs><filter id="hwMorpho" x="-25%" y="-25%" width="150%" height="150%"><feMorphology operator="${op}" radius="${r}"/></filter></defs>`;
      s=s.replace(/(<svg\b[^>]*>)([\s\S]*)(<\/svg>)/i,(_,o,c,cl)=>`${o}${filt}<g filter="url(#hwMorpho)">${c}</g>${cl}`);
    }
    return 'data:image/svg+xml;utf8,'+encodeURIComponent(s);
  }catch(_){ return iconSvgToDataUrl(rawSvg,baseColor,null); }
}
// 인덱스 캔버스: 각 요소를 고유 색으로 렌더 → 픽셀 샘플로 요소 식별
async function _buildIndexCanvas(rawSvg, els, size){
  try{
    const doc=new DOMParser().parseFromString(rawSvg,'image/svg+xml');
    const root=doc.querySelector('svg'); if(!root) return null;
    const docEls=iconColorableEls(doc);
    // ★defs/clipPath/mask/symbol 내부는 절대 건드리지 않음 — display:none을 주면 그걸 참조하는 clip-path/mask가
    // 브라우저에 따라 무효화(빈 클립 = 전부 안 보임)돼서 인덱스 색이 통째로 안 그려지는 버그가 생김.
    const idxMap=new Map();
    docEls.forEach((el,i)=>{
      const r=Math.floor(i/65536)%256, g=Math.floor(i/256)%256, b=i%256;
      el.setAttribute('fill',`rgb(${r},${g},${b})`); el.setAttribute('stroke',`rgb(${r},${g},${b})`);
      el.setAttribute('stroke-width','2'); el.setAttribute('opacity','1'); el.setAttribute('fill-opacity','1');
      el.removeAttribute('display'); el.removeAttribute('visibility');
      idxMap.set(`${r},${g},${b}`,i);
    });
    // 색칠 대상이 아닌 나머지 도형(장식·배경)만 색을 지워 안 보이게 — defs 내부 요소는 건드리지 않음
    [...doc.querySelectorAll('path,circle,rect,ellipse,polygon,polyline,line,text,tspan')].forEach(el=>{
      if(docEls.includes(el)) return;
      let p=el.parentElement, inDefs=false;
      while(p){ const t=(p.tagName||'').toLowerCase(); if(t==='defs'||t==='clippath'||t==='mask'||t==='symbol'){ inDefs=true; break; } p=p.parentElement; }
      if(inDefs) return;
    ...
    return await new Promise(res=>{
      const img=new Image();
      img.onload=()=>{ const c=document.createElement('canvas'); c.width=size; c.height=size; c.getContext('2d').drawImage(img,0,0); res({c,idxMap}); };
      img.onerror=()=>res(null);
      img.src='data:image/svg+xml;utf8,'+encodeURIComponent(svgStr);
    });
  }catch(_){ return null; }
}
// fill 아이콘을 강제 선 취급 → feMorphology dilate/erode로 두께 시뮬레이션
function iconSvgForcedStroke(rawSvg, color, stroke, parts){
  let s=rawSvg;
  // parts 적용
  if(parts&&Object.keys(parts).length){
    try{
      const doc=new DOMParser().parseFromString(rawSvg,'image/svg+xml');
      const root=doc.querySelector('svg'); if(root){
        const rs=_iconRootStroke(root),base=color||'#333333',els=iconColorableEls(doc);
        els.forEach(el2=>{ const ck=_elOrigColor(el2,rs,base); if(parts[ck]!=null) applyPartColor(el2,rs,parts[ck]); });
        s=new XMLSerializer().serializeToString(doc);
      }
    }catch(_){}
  }
  if(color) s=s.replace(/currentColor/g,color);
  const delta=(stroke||2)-2; // 0=원본, 양수=두껍게, 음수=얇게
  const radius=Math.abs(delta);
  if(radius>0.05){
    const op=delta>0?'dilate':'erode';
    const filt=`<defs><filter id="hwMorpho"><feMorphology in="SourceGraphic" operator="${op}" radius="${radius.toFixed(2)}"/></filter></defs>`;
    s=s.replace(/(<svg\b[^>]*>)([\s\S]*)(<\/svg>)/i,(_,open,content,close)=>`${open}${filt}<g filter="url(#hwMorpho)">${content}</g>${close}`);
  }
  return 'data:image/svg+xml;utf8,'+encodeURIComponent(s);
}
function refreshIconEl(e){
  if(!e||!e._iconSvg) return;
  // 두께: _iconThick(0=원본) → feMorphology로 fill·stroke 부분 함께 균일 조절
  e.src = renderIconWithParts(e._iconSvg, e._iconColor||'#333333', e._iconParts, e._iconThick||0);
}
// AI로 fill→stroke SVG 변환
async function aiConvertToStroke(e){
  if(!e||!e._iconSvg) return;
  const btn=document.getElementById('ic-ai-stroke'); if(btn){ btn.disabled=true; btn.textContent='🤖 변환 중…'; }
  try{
    const res=await aiProxy({ body:{
      model:'claude-sonnet-4-6', max_tokens:4000,
      system:`You are an SVG converter. Convert fill-based SVG icons to stroke-based SVG icons.
Rules:
- Replace fill="currentColor" shapes with stroke="currentColor" fill="none" equivalents
- For paths that are thin elongated filled shapes (they look like lines), trace their visual centerline and redraw as stroked paths
- Use stroke-width="2", stroke-linecap="round", stroke-linejoin="round" on all stroke elements
- Keep the same viewBox, overall structure, and proportions
- Remove unnecessary defs, masks, or complex structures if they only served fill rendering
- Return ONLY the SVG XML, no explanation, no markdown, no code fences`,
      messages:[{role:'user',content:`Convert this SVG icon to stroke-based:\n\n${e._iconSvg}`}]
    }});
    const newSvg=(res.data?.content?.[0]?.text||'').trim();
    if(!newSvg.includes('<svg')) throw new Error('변환 결과가 SVG가 아닙니다');
    const wasForcedStroke=e._iconStrokeForced;
    e._iconSvg=newSvg;
    e._iconStrokeForced=false;
    e._iconStrokeable=true;
    if(e._iconStroke==null) e._iconStroke=iconStrokeOf(newSvg)||2;
    refreshIconEl(e); renderCanvas(); save(true); renderProps();
    toast('✅ 선 아이콘으로 변환 완료!');
  }catch(err){
    toast('AI 변환 실패: '+(err.message||err));
    if(btn){ btn.disabled=false; btn.textContent='🤖 AI로 선 아이콘으로 변환'; }
  }
}
// 🤖 한 덩어리로 붙은 아이콘을 AI가 부분별 개별 path로 다시 구성 — 렌더 비교로 크게 달라지면 취소
async function aiSplitIcon(e, btn){
  if(!e||!e._iconSvg) return;
  const orig=e._iconSvg;
  if(btn){ btn.disabled=true; btn.textContent='🤖 AI가 부분을 나누는 중… (10~20초)'; }
  try{
    // 현재 아이콘 이미지(AI가 형태 파악용) — width/height는 DOM으로 덮어써 중복 속성 방지
    const SZ=200;
    let sstr=orig;
    try{ const d0=new DOMParser().parseFromString(orig,'image/svg+xml'); const r0=d0.querySelector('svg'); if(r0){ r0.setAttribute('width',SZ); r0.setAttribute('height',SZ); sstr=new XMLSerializer().serializeToString(d0);} }catch(_){}
    sstr=sstr.replace(/currentColor/g,'#333');
    const dat='data:image/svg+xml;utf8,'+encodeURIComponent(sstr);
    const tmp=new Image();
    const _ok=await new Promise(r=>{ tmp.onload=()=>r(true); tmp.onerror=()=>r(false); tmp.src=dat; });
    if(!_ok) throw new Error('아이콘 이미지를 만들지 못했습니다');
    const tc=document.createElement('canvas'); tc.width=SZ; tc.height=SZ; tc.getContext('2d').drawImage(tmp,0,0,SZ,SZ);
    const b64=tc.toDataURL('image/png').split(',')[1];
    const res=await aiProxy({ body:{ model:'claude-sonnet-4-6', max_tokens:4000,
      system:`You restructure SVG icons so each visually distinct part becomes a SEPARATE <path> element that can be colored independently.
Rules:
- Split the artwork into its distinct visual parts (e.g. cap, body, label, drop, handle). Each part = one <path>.
- The COMBINED rendering MUST look essentially identical to the original — same viewBox, same silhouette, same holes.
- Preserve holes using fill-rule="evenodd" on paths that need them.
- Use fill="currentColor" on every path (no hardcoded colors).
- Do NOT add strokes, filters, defs, masks, or clipPaths. Only <svg> and <path> elements.
- Prefer 2-5 parts. If the icon truly can't be meaningfully split, still return valid SVG.
- Return ONLY the SVG XML — no explanation, no markdown, no code fences.`,
      messages:[{ role:'user', content:[
        {type:'image',source:{type:'base64',media_type:'image/png',data:b64}},
        {type:'text',text:`Split this icon into separate colorable parts. Original SVG:\n\n${orig}`}
      ]}]
    }});
    let newSvg=(res.data?.content?.[0]?.text||'').trim();
    newSvg=newSvg.replace(/^```[a-z]*\s*/i,'').replace(/```$/,'').trim();
    if(!/<svg[\s\S]*<\/svg>/i.test(newSvg)) throw new Error('AI가 SVG를 반환하지 않았습니다');
    // 렌더 비교 — 원본과 너무 다르면(형태 깨짐) 취소
    const [a,bb]=await Promise.all([_rasterizeAlpha(orig,100),_rasterizeAlpha(newSvg,100)]);
    if(a&&bb){ let diff=0; for(let i=0;i<a.length;i+=4){ if(Math.abs(a[i+3]-bb[i+3])>40) diff++; } if(diff/(100*100)>0.06) throw new Error('AI 분리 결과가 원본과 달라 취소했어요 (모양 손상)'); }
    const cnt=iconPartCount(newSvg);
    _iconPushUndo(e);
    e._iconSvg=newSvg; e._iconParts={}; e._iconSplitV=999; // AI 결과는 재분해 안 함
    e._iconStrokeable=iconStrokeable(newSvg); e._iconMono=iconMono(newSvg);
    refreshIconEl(e); renderCanvas(); save(true);
    if(_partsEl===e) renderPartsEditor(); else _updateUndoBtn();
    toast(`✅ AI가 ${cnt}개 부분으로 나눴어요 — 칠해서 색 지정해보세요`);
  }catch(err){
    e._iconSvg=orig;
    toast('AI 분리 실패: '+(err.message||err));
    if(btn){ btn.disabled=false; btn.textContent='🤖 AI로 부분 나누기 — 잘 안 나뉠 때'; }
  }
}

// path의 d 문자열을 M/m(대·소문자 둘 다 새 하위경로 시작) 기준으로 쪼개고,
// 소문자 m(상대좌표)은 브라우저의 실제 경로 계산(getPointAtLength)으로 절대좌표를 구해 되돌린다.
// → 문자열만 자르면 상대좌표라 좌표가 깨지므로, 각 조각을 "M 절대x,절대y ...나머지 그대로" 형태로 재구성.
function _absolutizeSubpaths(d){
  d=(d||'').trim(); if(!d) return null;
  const idxs=[]; for(let i=0;i<d.length;i++){ const c=d[i]; if(c==='M'||c==='m') idxs.push(i); }
  if(idxs.length<2) return null;
  const NS='http://www.w3.org/2000/svg';
  const tmp=document.createElementNS(NS,'path');
  const numRe=/^[Mm]\s*([-+0-9.eE]+)[\s,]+([-+0-9.eE]+)/;
  const out=[];
  for(let i=0;i<idxs.length;i++){
    const start=idxs[i], end=(i+1<idxs.length)?idxs[i+1]:d.length;
    const raw=d.slice(start,end).trim();
    const m=raw.match(numRe); if(!m){ out.push(raw); continue; }
    const dx=parseFloat(m[1]), dy=parseFloat(m[2]);
    let ax=dx, ay=dy; // i===0: 첫 이동은 대·소문자 상관없이 절대 취급(스펙)
    if(i>0 && raw[0]==='m'){
      const prefix=d.slice(0,start);
      try{
        tmp.setAttribute('d', prefix);
        const len=tmp.getTotalLength();
        const p=tmp.getPointAtLength(len);
        ax=p.x+dx; ay=p.y+dy;
      }catch(_){ return null; } // 계산 실패 시 이 path는 건드리지 않음(안전)
    }
    const rest=raw.slice(m[0].length);
    out.push('M'+ax+','+ay+rest);
  }
  return out;
}
// 뭉친 path를 하위경로(subpath) 단위로 분해 → 선택 단위를 "눈에 보이는 조각"에 맞춤.
// 구멍(도넛의 안쪽처럼 시작점이 다른 조각 내부에 있는 조각)은 그 조각과 한 path로 묶어 구멍 유지.
function _splitIconSubpaths(svg){
  try{
    const doc=new DOMParser().parseFromString(svg,'image/svg+xml');
    const root=doc.querySelector('svg'); if(!root) return svg;
    let changed=false;
    const ctx=document.createElement('canvas').getContext('2d');
    iconColorableEls(doc).forEach(el=>{
      if((el.tagName||'').toLowerCase()!=='path') return;
      const d=el.getAttribute('d')||'';
      let segs=_absolutizeSubpaths(d);                    // M/m 둘 다 기준으로 절대좌표 분해
      if(!segs||segs.length<2) return;
      // fill 아이콘 판정(this 요소~조상까지 상속 fill)
      let strokeOnly=false;
      for(let node=el; node && node.nodeType===1; node=node.parentElement){
        const f=node.getAttribute && node.getAttribute('fill');
        if(f!=null){ strokeOnly=(f==='none'); break; }
      }
      // 그룹핑: 선 아이콘은 각 획이 독립 → 그룹 없음. 면 아이콘은 "다른 조각 안에 든 조각=구멍"을 원래 조각과 묶어 evenodd 유지
      const group=segs.map((_,i)=>i);
      const find=i=>group[i]===i?i:(group[i]=find(group[i]));
      if(!strokeOnly){
        const starts=segs.map(s=>{ const m=s.match(/^M\s*([-\d.eE+]+)[\s,]+([-\d.eE+]+)/); return m?{x:+m[1],y:+m[2]}:null; });
        for(let i=0;i<segs.length;i++){ if(!starts[i]) continue;
          for(let j=0;j<segs.length;j++){ if(i===j) continue;
            try{ if(ctx.isPointInPath(new Path2D(segs[j]), starts[i].x, starts[i].y)){ group[find(i)]=find(j); break; } }catch(_){}
          }
        }
      }
      const buckets=new Map();
      segs.forEach((s,i)=>{ const g=find(i); if(!buckets.has(g)) buckets.set(g,[]); buckets.get(g).push(s); });
      if(buckets.size<2) return;                          // 실질적으로 1덩어리면 분리 무의미
      const parent=el.parentNode;
      [...buckets.values()].forEach(arr=>{
        const c=el.cloneNode(false); c.setAttribute('d', arr.join(' ')); parent.insertBefore(c, el);
      });
...
// SVG를 지정 크기 캔버스에 그려 알파(모양) 픽셀 데이터를 반환 — 분해 전후 모양 비교용
function _rasterizeAlpha(svgStr, size){
  return new Promise(res=>{
    let s=svgStr;
    try{ const d=new DOMParser().parseFromString(svgStr,'image/svg+xml'); const r=d.querySelector('svg'); if(r){ r.setAttribute('width',size); r.setAttribute('height',size); s=new XMLSerializer().serializeToString(d);} }catch(_){}
    s=s.replace(/currentColor/g,'#000');
    const img=new Image();
    img.onload=()=>{ try{ const c=document.createElement('canvas'); c.width=size; c.height=size; const cx=c.getContext('2d'); cx.clearRect(0,0,size,size); cx.drawImage(img,0,0,size,size); res(cx.getImageData(0,0,size,size).data); }catch(_){ res(null); } };
    img.onerror=()=>res(null);
    img.src='data:image/svg+xml;utf8,'+encodeURIComponent(s);
  });
}
// 최대한 쪼개되, 분해본이 원본과 모양이 크게 달라지면(=깨짐) 원본을 유지 — "깨지지 않는 한도 내 최대 분해"
async function _splitIconSafe(svg){
  let split; try{ split=_splitIconSubpaths(svg); }catch(_){ return svg; }
  if(!split || split===svg) return svg;
  const SZ=100;
  let a,b; try{ [a,b]=await Promise.all([_rasterizeAlpha(svg,SZ),_rasterizeAlpha(split,SZ)]); }catch(_){ return svg; }
  if(!a||!b) return svg;                                  // 렌더 실패 → 안전하게 원본
  let diff=0; for(let i=0;i<a.length;i+=4){ if(Math.abs(a[i+3]-b[i+3])>40) diff++; }
  const ratio=diff/(SZ*SZ);
  return ratio>0.02 ? svg : split;                        // 모양이 2% 넘게 달라지면 원본 유지
}

// ── ✂️ 직접 나누기(칼선) 지오메트리 헬퍼 — 사용자가 그은 선으로 정사각 캔버스를 두 영역으로 정확히 분할 ──
// 점(px,py)에서 방향(dx,dy)으로 정사각(0~size) 경계까지 연장한 교차점
function _extendToBox(px,py,dx,dy,size){
  const cands=[];
  if(dx>1e-9){ const t=(size-px)/dx; const y=py+dy*t; if(t>0&&y>=-0.5&&y<=size+0.5) cands.push({t,x:size,y:Math.max(0,Math.min(size,y))}); }
  if(dx<-1e-9){ const t=(0-px)/dx; const y=py+dy*t; if(t>0&&y>=-0.5&&y<=size+0.5) cands.push({t,x:0,y:Math.max(0,Math.min(size,y))}); }
  if(dy>1e-9){ const t=(size-py)/dy; const x=px+dx*t; if(t>0&&x>=-0.5&&x<=size+0.5) cands.push({t,x:Math.max(0,Math.min(size,x)),y:size}); }
  if(dy<-1e-9){ const t=(0-py)/dy; const x=px+dx*t; if(t>0&&x>=-0.5&&x<=size+0.5) cands.push({t,x:Math.max(0,Math.min(size,x)),y:0}); }
  if(!cands.length) return {x:px,y:py};
  cands.sort((a,b)=>a.t-b.t);
  return {x:cands[0].x,y:cands[0].y};
}
// 정사각 경계 위의 점을 "둘레 위치"(0~4, 시계방향: 위→오른쪽→아래→왼쪽)로 변환
function _perimT(x,y,size){
  const eps=0.6;
  if(Math.abs(y-0)<=eps) return x/size;
  if(Math.abs(x-size)<=eps) return 1+y/size;
  if(Math.abs(y-size)<=eps) return 2+(size-x)/size;
  if(Math.abs(x-0)<=eps) return 3+(size-y)/size;
  const dTop=y, dRight=size-x, dBot=size-y, dLeft=x, m=Math.min(dTop,dRight,dBot,dLeft);
  if(m===dTop) return x/size; if(m===dRight) return 1+y/size; if(m===dBot) return 2+(size-x)/size; return 3+(size-y)/size;
}
// tFrom→tTo 사이(시계방향)에 있는 정사각 모서리 점들만 순서대로 반환
function _cornersBetween(tFrom,tTo,size){
  const corners=[{t:0,x:0,y:0},{t:1,x:size,y:0},{t:2,x:size,y:size},{t:3,x:0,y:size}];
  let span=tTo-tFrom; if(span<0) span+=4;
  return corners.filter(c=>{ let rel=c.t-tFrom; if(rel<0) rel+=4; return rel>1e-6&&rel<span-1e-6; })
    .sort((a,b)=>{ let ra=a.t-tFrom; if(ra<0)ra+=4; let rb=b.t-tFrom; if(rb<0)rb+=4; return ra-rb; })
    .map(c=>({x:c.x,y:c.y}));
}
function _svgViewBox(root){
  const vb=(root.getAttribute('viewBox')||'').trim().split(/[\s,]+/).map(Number);
  if(vb.length>=4 && vb.every(n=>!isNaN(n))) return {x:vb[0],y:vb[1],w:vb[2]||24,h:vb[3]||24};
  const w=parseFloat(root.getAttribute('width'))||24, h=parseFloat(root.getAttribute('height'))||24;
  return {x:0,y:0,w,h};
}
// 드래그(a→b)로 사각형/타원 닫힌 다각형 만들기 — 자르기 도구 공용(아이콘 나누기 · 영역 잘라내기).
// 첫 점을 마지막에 다시 붙여 확실히 "닫힌 영역"으로 판정되게 함(둘레 위치 판정용 closeDist=0 보장).
function _rectPts(a,b){
  const x0=Math.min(a.x,b.x),x1=Math.max(a.x,b.x),y0=Math.min(a.y,b.y),y1=Math.max(a.y,b.y);
  const p=[{x:x0,y:y0},{x:x1,y:y0},{x:x1,y:y1},{x:x0,y:y1}];
  p.push({x:p[0].x,y:p[0].y});
  return p;
}
function _ellipsePts(a,b,n){
  n=n||40;
  const x0=Math.min(a.x,b.x),x1=Math.max(a.x,b.x),y0=Math.min(a.y,b.y),y1=Math.max(a.y,b.y);
  const cx=(x0+x1)/2, cy=(y0+y1)/2, rx=Math.max(1,(x1-x0)/2), ry=Math.max(1,(y1-y0)/2);
  const pts=[];
  for(let i=0;i<n;i++){ const t=i/n*Math.PI*2; pts.push({x:cx+rx*Math.cos(t), y:cy+ry*Math.sin(t)}); }
  pts.push({x:pts[0].x,y:pts[0].y});
  return pts;
}

// ── 부분 색 편집기 (직접 클릭·드래그 방식) ──
let _partsEl=null, _selPart=null; // _selPart = origColorKey string
let _iconPartBrushColor='#3b82f6'; // 부분 색칠에서 '칠할 색'(앱 색상 피커와 연동)
let _iconPartsMouseUpHandler=null; // window 리스너 — 매번 재등록 시 이전 것 제거(누수 방지)
let _iconUndoStack=[]; // 부분 색칠 창의 되돌리기 스택 — [{svg,parts,color,thick}, ...] (구형 부분나누기·색적용 전 스냅샷)
function _iconSnapshot(e){ return { svg:e._iconSvg, parts:JSON.parse(JSON.stringify(e._iconParts||{})), color:e._iconColor, thick:e._iconThick||0 }; }
function _iconPushUndo(e){ _iconUndoStack.push(_iconSnapshot(e)); if(_iconUndoStack.length>30) _iconUndoStack.shift(); }
function _iconUndo(e){
  const snap=_iconUndoStack.pop(); if(!snap){ toast('더 되돌릴 내용이 없어요'); return; }
  e._iconSvg=snap.svg; e._iconParts=snap.parts; e._iconColor=snap.color; e._iconThick=snap.thick;
  refreshIconEl(e); renderCanvas(); save(true);
  if(_partsEl===e) renderPartsEditor();
  toast('↩ 되돌렸어요');
}
function _updateUndoBtn(){ const b=document.getElementById('iconparts-undo'); if(b) b.disabled=_iconUndoStack.length===0; }
async function openIconParts(){
  const e=selId?el(selId):null; if(!e||!e._iconSvg) return;
  _partsEl=e; _selPart=null; _iconUndoStack=[];
  document.getElementById('iconparts-modal').style.display='flex';
  // 뭉친 path를 조각 단위로 1회 분해(영구 반영) — 분해되면 기존 part 인덱스가 바뀌므로 parts 초기화
  // _iconSplitV: 분해 로직 버전(로직 개선 시 올려서 기존 아이콘도 재분해되게 함)
  const SPLIT_V=5; // v5: 선·면 모두 최대 분해 + 렌더 검증으로 깨지면 원본 유지
  if(e._iconSplitV!==SPLIT_V){
    const prev=document.getElementById('iconparts-preview'); if(prev) prev.innerHTML='<div style="font-size:12px;color:var(--sub);padding:16px;text-align:center">아이콘 조각 분석 중…</div>';
    _iconPushUndo(e);
    const s2=await _splitIconSafe(e._iconSvg);
    if(s2!==e._iconSvg){ e._iconSvg=s2; e._iconParts={}; refreshIconEl(e); } else _iconUndoStack.pop();
    e._iconSplitV=SPLIT_V; save(true);
    if(_partsEl!==e) return;                              // 분석 중 다른 걸 열었으면 중단
  }
  // 구형 숫자키 _iconParts 초기화
  if(e._iconParts&&Object.keys(e._iconParts).some(k=>!isNaN(k))) e._iconParts={};
  else e._iconParts=e._iconParts||{};
  renderPartsEditor();
}
function _rebuildSvgWrap(e, rs, base){
  const doc2=new DOMParser().parseFromString(e._iconSvg,'image/svg+xml');
  const root2=doc2.querySelector('svg'); if(!root2) return null;
  iconColorableEls(doc2).forEach(el2=>{
    const ck=_elOrigColor(el2,rs,base); el2.setAttribute('data-ck',ck);
    if(e._iconParts[ck]!=null) applyPartColor(el2,rs,e._iconParts[ck]);
  });
  root2.setAttribute('width','190'); root2.setAttribute('height','190');
  let s=new XMLSerializer().serializeToString(doc2);
  return s.replace(/currentColor/g,base);
}
function renderPartsEditor(){
  const e=_partsEl; if(!e) return;
  const prev=document.getElementById('iconparts-preview'), list=document.getElementById('iconparts-list');
  if(!prev||!list) return;
  _updateUndoBtn();
  // 레거시 색상 키 parts 마이그레이션 (숫자 인덱스 또는 색상 키 → part:N 형식)
  if(e._iconParts && Object.keys(e._iconParts).some(k=>!k.startsWith('part:'))) e._iconParts={};
  else e._iconParts=e._iconParts||{};

  if(!e._iconSvg){ prev.innerHTML='<div style="font-size:12px;color:var(--sub);padding:16px;text-align:center">이 아이콘엔 편집 가능한 원본 데이터가 없어요.<br>(예전 버전에서 삽입된 아이콘이거나 일반 이미지)</div>'; list.innerHTML=''; return; }
  let doc, root;
  try{ doc=new DOMParser().parseFromString(e._iconSvg,'image/svg+xml'); root=doc.querySelector('svg'); }catch(err){ root=null; }
  if(!root || doc.querySelector('parsererror')){
    prev.innerHTML='<div style="font-size:12px;color:var(--sub);padding:16px;text-align:center">⚠ 아이콘 SVG를 읽지 못했습니다.<br>다른 아이콘으로 다시 시도해 주세요.</div>';
    list.innerHTML='';
    return;
  }
  const rs=_iconRootStroke(root), base=e._iconColor||'#333333';
  if(!_iconPartBrushColor) _iconPartBrushColor=base;      // 최초 기본 '칠할 색' = 아이콘 현재 색
  const els=iconColorableEls(doc);
  if(!els.length){ prev.innerHTML='<div style="font-size:12px;color:var(--sub);padding:16px;text-align:center">색칠 가능한 부분을 찾지 못했어요.</div>'; list.innerHTML=''; return; }
  try{
  const SZ=190;
  let _brushSz=6, _painting=false, _hitIdxs=new Set(), _indexData=null;

  // 아이콘 현재 상태(parts 적용) img src
  function _currentIconUrl(){ return renderIconWithParts(e._iconSvg,base,e._iconParts,e._iconThick||0); }

  // preview: 아이콘 img + 드로잉 canvas
  prev.style.cssText=`position:relative;width:${SZ}px;height:${SZ}px;margin:0 auto 12px;background:var(--panel2);border:1px solid var(--border);border-radius:12px;overflow:hidden;flex-shrink:0`;
  prev.innerHTML=`<img id="iparts-img" src="${_currentIconUrl()}" style="position:absolute;inset:0;width:${SZ}px;height:${SZ}px;object-fit:contain;pointer-events:none"><canvas id="iparts-draw" width="${SZ}" height="${SZ}" style="position:absolute;inset:0;cursor:crosshair"></canvas>`;

  const dcvs=document.getElementById('iparts-draw');
  const dctx=dcvs.getContext('2d');

  // 인덱스 캔버스 비동기 빌드
  let _indexFailed=false;
  _buildIndexCanvas(e._iconSvg,els,SZ).then(d=>{
    _indexData=d;
    if(!d){ _indexFailed=true; const st=document.getElementById('iparts-stat'); if(st) st.textContent='⚠ 이 아이콘은 부분 인식 준비에 실패했어요'; }
  });

  function _sampleAt(x,y,mode){
    if(!_indexData) return;
    const bx=Math.max(0,Math.round(x-_brushSz/2)), by=Math.max(0,Math.round(y-_brushSz/2));
    const bw=Math.min(_brushSz,SZ-bx), bh=Math.min(_brushSz,SZ-by);
    if(bw<=0||bh<=0) return;
    let px;
    try{ px=_indexData.c.getContext('2d').getImageData(bx,by,bw,bh).data; }
    catch(_){ _indexFailed=true; const st=document.getElementById('iparts-stat'); if(st) st.textContent='⚠ 이 아이콘은 브라우저 보안 제한으로 부분 인식이 안 돼요'; return; }
    for(let i=0;i<px.length;i+=4){
      if(px[i+3]<10) continue;
      const k=`${px[i]},${px[i+1]},${px[i+2]}`;
      const idx=_indexData.idxMap.get(k);
      if(idx===undefined) continue;
      if(mode==='sub') _hitIdxs.delete(idx); else _hitIdxs.add(idx);
    }
  }

  let _brushMode='add'; // 'add'=선택 추가(주황), 'sub'=선택에서 빼기(파랑), 'cut'=직접 나누기(자홍 선)
  let _cutPts=[];
  let _cutShape='poly'; // 나누기 도형: 'poly'=점찍기, 'rect'=사각형(드래그), 'ellipse'=원형(드래그)
  let _cutDragStart=null;
  function _paintAt(x,y){
    dctx.globalCompositeOperation='source-over';
    dctx.fillStyle=_brushMode==='sub'?'rgba(40,120,255,0.38)':'rgba(255,80,0,0.38)';
    dctx.beginPath(); dctx.arc(x,y,_brushSz/2,0,Math.PI*2); dctx.fill();
    _sampleAt(x,y,_brushMode);
    _updateHilite();
  }
  // 나누기 미리보기 — 매번 지우고 전체 선(+poly는 점 표시)을 다시 그림
  function _redrawCutPreview(pts){
    dctx.clearRect(0,0,SZ,SZ);
    dctx.strokeStyle='rgba(255,0,150,0.95)'; dctx.lineWidth=2; dctx.lineJoin='round'; dctx.lineCap='round';
    if(pts.length>1){ dctx.beginPath(); dctx.moveTo(pts[0].x,pts[0].y); for(let i=1;i<pts.length;i++) dctx.lineTo(pts[i].x,pts[i].y); dctx.stroke(); }
    if(_cutShape==='poly'){ dctx.fillStyle='rgba(255,0,150,0.95)'; _cutPts.forEach(p=>{ dctx.beginPath(); dctx.arc(p.x,p.y,3.5,0,Math.PI*2); dctx.fill(); }); }
  }
  function _updatePolyBtns(){
    const done=document.getElementById('iparts-poly-done'), cancel=document.getElementById('iparts-poly-cancel');
    const isCutLike=(_brushMode==='cut'&&_cutShape==='poly');
    const show=isCutLike&&_cutPts.length>0;
    if(done) done.style.display=(isCutLike&&_cutPts.length>=2)?'inline-block':'none';
    if(cancel) cancel.style.display=show?'inline-block':'none';
  }
  // 그은 선이 지나는 지점들의 인덱스 캔버스 색을 읽어 어느 조각을 지나는지 판정
  function _indicesAlongLine(points){
    const set=new Set(); if(!_indexData) return set;
    const ictx=_indexData.c.getContext('2d');
    points.forEach(p=>{
      const x=Math.max(0,Math.min(SZ-1,Math.round(p.x))), y=Math.max(0,Math.min(SZ-1,Math.round(p.y)));
      try{ const d=ictx.getImageData(x,y,1,1).data; if(d[3]>=10){ const k=`${d[0]},${d[1]},${d[2]}`; const idx=_indexData.idxMap.get(k); if(idx!==undefined) set.add(idx); } }catch(_){}
    });
    return set;
  }
  // 그은 선으로 지나는 조각(들)을 실제로 둘로 클립 분할(벡터 유지, 안 깨짐)
  function _applyCut(){
    const pts=_cutPts;
    if(pts.length<2){ dctx.clearRect(0,0,SZ,SZ); _cutPts=[]; _updatePolyBtns(); return; }
    let crossed=_indicesAlongLine(pts);
    // 픽셀 감지가 안 되는 아이콘(내가 직접 가져온 SVG 등)도 있으므로, 못 찾았다고 막지 않고 전체 요소에 적용
    // (클리핑 자체는 좌표 기반이라 정확히 동작 — crossed는 "어디를 감쌀지"의 힌트일 뿐 필수 조건이 아님)
    if(!crossed.size) crossed=new Set(els.map((_,i)=>i));
    _iconPushUndo(e);
    try{
      // 시작점~끝점이 가까우면(올가미처럼 닫힘) "안쪽 면적만" 떼어내기, 아니면 기존처럼 경계까지 연장한 칼선 나누기
      const first=pts[0], last=pts[pts.length-1];
      const closeDist=Math.hypot(first.x-last.x, first.y-last.y);
      const isArea = pts.length>=4 && closeDist<Math.max(14,SZ*0.07);
      const vb=_svgViewBox(root);
      const scl=Math.min(SZ/vb.w, SZ/vb.h), dispW=vb.w*scl, dispH=vb.h*scl, offX=(SZ-dispW)/2, offY=(SZ-dispH)/2;
      const toVb=p=>({x:(p.x-offX)/scl+vb.x, y:(p.y-offY)/scl+vb.y});
      const ptsStr=arr=>arr.map(p=>{const v=toVb(p); return v.x.toFixed(2)+','+v.y.toFixed(2);}).join(' ');
      const svgNS='http://www.w3.org/2000/svg';
      let defsEl=root.querySelector('defs'); if(!defsEl){ defsEl=doc.createElementNS(svgNS,'defs'); root.insertBefore(defsEl, root.firstChild); }
      const mkId=()=>'cut'+Date.now().toString(36)+Math.random().toString(36).slice(2,7);
      const idA=mkId(), idB=mkId();
      const cpA=doc.createElementNS(svgNS,'clipPath'); cpA.setAttribute('id',idA);
      const cpB=doc.createElementNS(svgNS,'clipPath'); cpB.setAttribute('id',idB);
      if(isArea){
        // ✂️ 영역(올가미) 나누기 — 닫힌 선 안쪽=A, 나머지 전체(캔버스에서 안쪽을 구멍처럼 뺀 것)=B
        const polA=doc.createElementNS(svgNS,'polygon'); polA.setAttribute('points',ptsStr(pts)); cpA.appendChild(polA);
        const sq=[{x:0,y:0},{x:SZ,y:0},{x:SZ,y:SZ},{x:0,y:SZ}];
        const dPath='M'+ptsStr(sq).replace(/ /g,' L ')+' Z M'+ptsStr(pts).replace(/ /g,' L ')+' Z';
        const pathB=doc.createElementNS(svgNS,'path'); pathB.setAttribute('d',dPath); pathB.setAttribute('fill-rule','evenodd'); cpB.appendChild(pathB);
      } else {
        const p0=pts[0], p1=pts[1]||p0;
        let dx0=p0.x-p1.x, dy0=p0.y-p1.y; const n0=Math.hypot(dx0,dy0)||1; dx0/=n0; dy0/=n0;
        const startExt=_extendToBox(p0.x,p0.y,dx0,dy0,SZ);
        const pn=pts[pts.length-1], pn1=pts[pts.length-2]||pn;
        let dxe=pn.x-pn1.x, dye=pn.y-pn1.y; const ne=Math.hypot(dxe,dye)||1; dxe/=ne; dye/=ne;
        const endExt=_extendToBox(pn.x,pn.y,dxe,dye,SZ);
        const tStart=_perimT(startExt.x,startExt.y,SZ), tEnd=_perimT(endExt.x,endExt.y,SZ);
        const polyA=[startExt, ...pts, endExt, ..._cornersBetween(tEnd,tStart,SZ)];
        const polyB=[endExt, ...pts.slice().reverse(), startExt, ..._cornersBetween(tStart,tEnd,SZ)];
        const polA=doc.createElementNS(svgNS,'polygon'); polA.setAttribute('points',ptsStr(polyA)); cpA.appendChild(polA);
        const polB=doc.createElementNS(svgNS,'polygon'); polB.setAttribute('points',ptsStr(polyB)); cpB.appendChild(polB);
      }
      defsEl.appendChild(cpA); defsEl.appendChild(cpB);
      [...crossed].forEach(i=>{
        const orig=els[i]; if(!orig||!orig.parentNode) return;
        const cA=orig.cloneNode(true); cA.setAttribute('clip-path','url(#'+idA+')');
        const cB=orig.cloneNode(true); cB.setAttribute('clip-path','url(#'+idB+')');
        orig.parentNode.insertBefore(cA, orig); orig.parentNode.insertBefore(cB, orig); orig.parentNode.removeChild(orig);
      });
      e._iconSvg=new XMLSerializer().serializeToString(doc);
      e._iconParts={}; e._iconSplitV=999; // 인덱스가 바뀌므로 기존 부분색 초기화
      refreshIconEl(e); renderCanvas(); save(true);
      toast(isArea?`✂️ 안쪽 영역을 별도 부분으로 떼어냈어요 (${crossed.size}개)`:`✂️ ${crossed.size}개 조각을 나눴어요`);
      if(_partsEl===e) renderPartsEditor(); else _updateUndoBtn();
    }catch(err){ toast('나누기 실패: '+(err.message||err)); _iconUndoStack.pop(); }
    finally{ dctx.clearRect(0,0,SZ,SZ); _cutPts=[]; }
  }
  // 🗑 선택한 부분 삭제 — ✂️ 나누기로 조각낸 뒤, ➕ 추가로 칠해서 고른 조각(들)을 실제로 지움
  function _deleteSelected(){
    if(!_hitIdxs.size){ toast('먼저 지울 부분을 칠해서 선택하세요'); return; }
    _iconPushUndo(e);
    try{
      const n=_hitIdxs.size;
      [..._hitIdxs].sort((a,b)=>b-a).forEach(i=>{
        const orig=els[i]; if(!orig||!orig.parentNode) return;
        const parent=orig.parentNode;
        // 우리가 나누기로 만든 <g clip-path> 래퍼 안에 이 조각 하나뿐이면, 빈 래퍼가 안 남게 래퍼째 제거
        if(parent.tagName&&parent.tagName.toLowerCase()==='g'&&parent.getAttribute('clip-path')&&parent.children.length===1&&parent.parentNode){
      ...
      e._iconSvg=new XMLSerializer().serializeToString(doc);
      e._iconParts={}; e._iconSplitV=999; // 요소 개수·인덱스가 바뀌므로 초기화
      refreshIconEl(e); renderCanvas(); save(true);
      dctx.clearRect(0,0,SZ,SZ); _hitIdxs=new Set();
      toast(`🗑 ${n}개 삭제됨`);
      if(_partsEl===e) renderPartsEditor(); else _updateUndoBtn();
    }catch(err){ toast('삭제 실패: '+(err.message||err)); _iconUndoStack.pop(); }
  }

  function _updateHilite(){
    // 아이콘 이미지에서 선택 요소만 밝게 — SVG 재렌더
    if(_hitIdxs.size===0){ document.getElementById('iparts-img').src=_currentIconUrl(); return; }
    const doc2=new DOMParser().parseFromString(e._iconSvg,'image/svg+xml');
    const root2=doc2.querySelector('svg'); if(!root2) return;
    const els2=iconColorableEls(doc2);
    els2.forEach((el2,i)=>{
      if(_hitIdxs.has(i)) return; // 선택된 것만 원래 색
      el2.setAttribute('opacity','0.12');
    });
    // 기존 parts 적용
    els2.forEach((el2,i)=>{ const col=e._iconParts['part:'+i]; if(col) applyPartColor(el2,rs,col); });
    root2.setAttribute('width',SZ); root2.setAttribute('height',SZ);
    let s=new XMLSerializer().serializeToString(doc2); s=s.replace(/currentColor/g,base);
    document.getElementById('iparts-img').src='data:image/svg+xml;utf8,'+encodeURIComponent(s);
  }

  // 매 스트로크마다 선택을 초기화하지 않음 — 여러 번 나눠 칠하거나(추가) 잘못 잡힌 부분을 빼기(빼기 모드)로 계속 다듬을 수 있게.
  // 나누기 모드: poly=클릭으로 점을 하나씩 찍어 완료 버튼/더블클릭으로 확정, rect/ellipse=드래그로 그리고 놓으면 바로 적용
  const MIN_DRAG=6;
  dcvs.onmousedown=ev=>{
    ev.preventDefault();
    const r=dcvs.getBoundingClientRect(), x=ev.clientX-r.left, y=ev.clientY-r.top;
    if(_brushMode==='cut'){
      if(_cutShape==='poly'){ _cutPts.push({x,y}); _redrawCutPreview(_cutPts); _updatePolyBtns(); return; }
      _painting=true; _cutDragStart={x,y}; _cutPts=[{x,y}]; _redrawCutPreview(_cutPts);
      return;
    }
    _painting=true; _paintAt(x,y);
  };
  dcvs.onmousemove=ev=>{
    const r=dcvs.getBoundingClientRect(), x=ev.clientX-r.left, y=ev.clientY-r.top;
    if(_brushMode==='cut'){
      if(_cutShape==='poly'){ if(_cutPts.length) _redrawCutPreview([..._cutPts,{x,y}]); return; }
      if(!_painting||!_cutDragStart) return;
      _cutPts=_cutShape==='rect'?_rectPts(_cutDragStart,{x,y}):_ellipsePts(_cutDragStart,{x,y},40);
      _redrawCutPreview(_cutPts);
      return;
    }
    if(!_painting) return; _paintAt(x,y);
  };
  dcvs.ondblclick=ev=>{ if(_brushMode==='cut'&&_cutShape==='poly'&&_cutPts.length>=2){ ev.preventDefault(); _applyCut(); _updatePolyBtns(); } };
  // 캔버스 밖에서 마우스를 놓아도(흔한 케이스) 놓친 mouseup을 잡도록 window 전체에서 감지
  if(_iconPartsMouseUpHandler) window.removeEventListener('mouseup', _iconPartsMouseUpHandler);
  _iconPartsMouseUpHandler=ev=>{
    if(!_painting) return; _painting=false;
    if(_brushMode==='cut'){
      if(_cutShape==='poly') return;                                     // 클릭 기반이라 드래그 종료 처리 불필요
      const st=_cutDragStart, r=dcvs.getBoundingClientRect();
      const ex=(ev.clientX!=null?ev.clientX-r.left:st.x), ey=(ev.clientY!=null?ev.clientY-r.top:st.y);
      _cutDragStart=null;
      if(st && Math.hypot(ex-st.x, ey-st.y)>=MIN_DRAG) _applyCut();
      else { _cutPts=[]; dctx.clearRect(0,0,SZ,SZ); }
      return;
    }
    _updateUI();
  };
  window.addEventListener('mouseup', _iconPartsMouseUpHandler);

  function _updateUI(){
    const stat=document.getElementById('iparts-stat'), row=document.getElementById('iparts-arow'), aib=document.getElementById('iparts-ai');
    if(!stat) return;
    if(_hitIdxs.size>0){
      stat.textContent=`${_hitIdxs.size}개 요소 선택됨 — 색을 고르세요`;
      if(row) row.style.display='flex'; if(aib) aib.style.display='';
    } else if(_indexFailed){
      stat.textContent='⚠ 이 아이콘은 부분 인식이 안 돼요 — 색 전체는 여전히 바꿀 수 있어요';
      if(row) row.style.display='none'; if(aib) aib.style.display='none';
    } else {
      stat.textContent='아이콘 위에 칠하면 해당 부분이 선택됩니다';
      if(row) row.style.display='none'; if(aib) aib.style.display='none';
    }
  }
  ...
  list.innerHTML=`
    <div style="display:flex;align-items:center;gap:6px;width:100%;margin-bottom:6px">
      <button type="button" class="tb-btn ipm-btn on" id="iparts-mode-add" style="flex:1;font-size:11px">➕ 추가</button>
      <button type="button" class="tb-btn ipm-btn" id="iparts-mode-sub" style="flex:1;font-size:11px">➖ 빼기</button>
      <button type="button" class="tb-btn ipm-btn" id="iparts-mode-cut" style="flex:1;font-size:11px">✂️ 나누기</button>
    </div>
    <div id="iparts-cutshaperow" style="display:none;align-items:center;gap:5px;width:100%;margin-bottom:8px">
      <button type="button" class="tb-btn ics-btn on" id="iparts-cs-poly" style="flex:1;font-size:10px">📍 점찍기</button>
      <button type="button" class="tb-btn ics-btn" id="iparts-cs-rect" style="flex:1;font-size:10px">▭ 사각형</button>
      <button type="button" class="tb-btn ics-btn" id="iparts-cs-ellipse" style="flex:1;font-size:10px">⭕ 원형</button>
    </div>
    <div id="iparts-polybtns" style="display:none;gap:6px;width:100%;margin-bottom:8px">
      <button type="button" class="tb-btn primary" id="iparts-poly-done" style="flex:1;font-size:11px;display:none">✓ 나누기 완료 (더블클릭도 OK)</button>
      <button type="button" class="tb-btn" id="iparts-poly-cancel" style="flex-shrink:0;font-size:11px;display:none">✕ 취소</button>
    </div>
    <div id="iparts-brushrow" style="display:flex;align-items:center;gap:7px;width:100%;margin-bottom:8px">
      <span style="font-size:11px;color:var(--sub);white-space:nowrap">브러시</span>
      <input type="range" id="iparts-bsz" min="6" max="50" value="${_brushSz}" style="flex:1">
      <button type="button" class="tb-btn" id="iparts-clear" style="flex-shrink:0;font-size:11px;padding:3px 8px">↺ 지우기</button>
    </div>
    <div id="iparts-stat" style="font-size:12px;color:var(--sub);text-align:center;width:100%;padding:3px 0">➕ 추가 모드 — 원하는 부분을 칠하면 선택됩니다 (➖ 빼기로 잘못 잡힌 부분 제외 가능)</div>
    <div id="iparts-arow" style="display:none;align-items:center;gap:7px;width:100%;margin-top:8px;flex-wrap:wrap">
      <span style="font-size:12px;color:var(--sub);white-space:nowrap">칠할 색</span>
      <button type="button" class="panel-cbtn" id="iparts-cbtn" data-cpkey="iconPartBrushColor" title="칠할 색"><span id="iparts-col-sw" style="background:${_iconPartBrushColor}"></span></button>
      <button type="button" class="tb-btn primary" id="iparts-apply" style="flex:1">적용</button>
      <button type="button" class="tb-btn" id="iparts-erase">원래대로</button>
      <button type="button" class="tb-btn" id="iparts-delete" style="flex-basis:100%;color:#ff6b6b;border-color:rgba(255,107,107,.4)">🗑 선택한 부분 삭제 (나누기로 나뉜 조각을 완전히 제거)</button>
    </div>
    <button type="button" class="tb-btn" id="iparts-ai" style="display:none;width:100%;font-size:11px;margin-top:6px">🤖 AI 보정 — 인접 요소 자동 포함</button>
    <button type="button" class="tb-btn" id="iparts-aisplit" style="width:100%;font-size:11px;margin-top:8px;border-color:rgba(108,123,255,.4)">🤖 AI로 부분 나누기 — 잘 안 나뉠 때</button>
    <div style="font-size:10px;color:var(--sub);text-align:center;margin-top:3px">뚜껑·몸통처럼 한 덩어리로 붙은 아이콘을 AI가 부분별로 분리해요</div>
    <div id="iparts-chips" style="display:flex;flex-wrap:wrap;gap:5px;justify-content:center;margin-top:10px;width:100%"></div>`;

  const bszIn=document.getElementById('iparts-bsz'); if(bszIn) bszIn.oninput=()=>_brushSz=+bszIn.value;
  const clrBtn=document.getElementById('iparts-clear'); if(clrBtn) clrBtn.onclick=()=>{ dctx.clearRect(0,0,SZ,SZ); _hitIdxs=new Set(); document.getElementById('iparts-img').src=_currentIconUrl(); _updateUI(); };
  const modeAddBtn=document.getElementById('iparts-mode-add'), modeSubBtn=document.getElementById('iparts-mode-sub'), modeCutBtn=document.getElementById('iparts-mode-cut');
  function _cutStatText(){
    if(_cutShape==='rect') return '▭ 드래그로 사각형을 그리세요 — 손을 떼면 바로 잘려요';
    if(_cutShape==='ellipse') return '⭕ 드래그로 원형을 그리세요 — 손을 떼면 바로 잘려요';
    return '📍 클릭으로 점을 찍어 선을 그으세요 — 시작점 근처로 닫으면 안쪽 면적만 떼어내고, 열어두면 선으로 나눠요 (더블클릭 또는 완료 버튼으로 확정)';
  }
  function _setMode(m){
    _brushMode=m;
    if(modeAddBtn) modeAddBtn.classList.toggle('on', m==='add');
    if(modeSubBtn) modeSubBtn.classList.toggle('on', m==='sub');
    if(modeCutBtn) modeCutBtn.classList.toggle('on', m==='cut');
    const isCutLike=(m==='cut');
    const brow=document.getElementById('iparts-brushrow'); if(brow) brow.style.display=isCutLike?'none':'flex';
    const csrow=document.getElementById('iparts-cutshaperow'); if(csrow) csrow.style.display=isCutLike?'flex':'none';
    const pbtns=document.getElementById('iparts-polybtns'); if(pbtns) pbtns.style.display=(isCutLike&&_cutShape==='poly')?'flex':'none';
    _cutPts=[]; _cutDragStart=null; dctx.clearRect(0,0,SZ,SZ); _updatePolyBtns();
    const st=document.getElementById('iparts-stat');
    if(st && isCutLike) st.textContent=_cutStatText();
    else if(st && _hitIdxs.size===0) st.textContent='아이콘 위에 칠하면 해당 부분이 선택됩니다';
  }
  if(modeAddBtn) modeAddBtn.onclick=()=>_setMode('add');
  if(modeSubBtn) modeSubBtn.onclick=()=>_setMode('sub');
  if(modeCutBtn) modeCutBtn.onclick=()=>_setMode('cut');
  const csPolyBtn=document.getElementById('iparts-cs-poly'), csRectBtn=document.getElementById('iparts-cs-rect'), csEllipseBtn=document.getElementById('iparts-cs-ellipse');
  function _setCutShape(s){
    _cutShape=s; _cutPts=[]; _cutDragStart=null; dctx.clearRect(0,0,SZ,SZ);
    if(csPolyBtn) csPolyBtn.classList.toggle('on', s==='poly');
    if(csRectBtn) csRectBtn.classList.toggle('on', s==='rect');
    if(csEllipseBtn) csEllipseBtn.classList.toggle('on', s==='ellipse');
    const pbtns=document.getElementById('iparts-polybtns'); if(pbtns) pbtns.style.display=(s==='poly')?'flex':'none';
    _updatePolyBtns();
    const st=document.getElementById('iparts-stat'); if(st) st.textContent=_cutStatText();
  }
  if(csPolyBtn) csPolyBtn.onclick=()=>_setCutShape('poly');
  if(csRectBtn) csRectBtn.onclick=()=>_setCutShape('rect');
  if(csEllipseBtn) csEllipseBtn.onclick=()=>_setCutShape('ellipse');
  const polyDoneBtn=document.getElementById('iparts-poly-done'), polyCancelBtn=document.getElementById('iparts-poly-cancel');
  if(polyDoneBtn) polyDoneBtn.onclick=()=>{ if(_cutPts.length>=2) _applyCut(); _updatePolyBtns(); };
  const delBtn=document.getElementById('iparts-delete'); if(delBtn) delBtn.onclick=_deleteSelected;
  if(polyCancelBtn) polyCancelBtn.onclick=()=>{ _cutPts=[]; dctx.clearRect(0,0,SZ,SZ); _updatePolyBtns(); };
  const aiSplitBtn=document.getElementById('iparts-aisplit');
  if(aiSplitBtn) aiSplitBtn.onclick=()=>aiSplitIcon(e, aiSplitBtn);
  // '칠할 색' — 앱(홈탭) 색상 피커 연동
  if(typeof CP_TARGETS!=='undefined' && !CP_TARGETS.iconPartBrushColor){
    CP_TARGETS.iconPartBrushColor={ label:'칠할 색', rich:false,
      current:()=>_iconPartBrushColor,
      set:v=>{ _iconPartBrushColor=(v==='transparent'?'#000000':v); const sw=document.getElementById('iparts-col-sw'); if(sw) sw.style.background=_iconPartBrushColor; } };
  }

  function _renderChips(){
    const chips=document.getElementById('iparts-chips'); if(!chips) return;
    const keys=Object.keys(e._iconParts||{});
    chips.innerHTML=keys.length?'<div style="width:100%;font-size:11px;color:var(--sub);margin-bottom:4px;text-align:center">적용된 색 (클릭: 취소)</div>'+keys.map((k,i)=>`<div class="ipc-chip" data-i="${i}" style="width:22px;height:22px;border-radius:5px;background:${e._iconParts[k]};border:2px solid var(--border);cursor:pointer"></div>`).join(''):'';
    chips.querySelectorAll('.ipc-chip').forEach(b=>b.onclick=()=>{
      const k2=Object.keys(e._iconParts)[+b.dataset.i]; if(!k2) return;
      delete e._iconParts[k2]; refreshIconEl(e); renderCanvas(); save(true);
      document.getElementById('iparts-img').src=_currentIconUrl(); _renderChips();
    });
  }
  _renderChips();

  const ab=document.getElementById('iparts-apply');
  if(ab) ab.onclick=()=>{
    _iconPushUndo(e);
    const col=_iconPartBrushColor||base;
    [..._hitIdxs].forEach(i=>{ e._iconParts['part:'+i]=col; });
    refreshIconEl(e); renderCanvas(); save(true);
    dctx.clearRect(0,0,SZ,SZ); _hitIdxs=new Set();
    document.getElementById('iparts-img').src=_currentIconUrl(); _updateUI(); _renderChips(); _updateUndoBtn();
  };
  const eb=document.getElementById('iparts-erase');
  if(eb) eb.onclick=()=>{
    _iconPushUndo(e);
    [..._hitIdxs].forEach(i=>delete e._iconParts['part:'+i]);
    refreshIconEl(e); renderCanvas(); save(true);
    dctx.clearRect(0,0,SZ,SZ); _hitIdxs=new Set();
    document.getElementById('iparts-img').src=_currentIconUrl(); _updateUI(); _renderChips(); _updateUndoBtn();
  };

  // 🤖 AI 보정: 그린 내용 + 아이콘 이미지 → Claude가 인접/연관 요소 추가
  const aib=document.getElementById('iparts-ai');
  if(aib) aib.onclick=async()=>{
    aib.disabled=true; aib.textContent='🤖 분석 중…';
    try{
      const iconImg=document.getElementById('iparts-img');
      const tc=document.createElement('canvas'); tc.width=SZ; tc.height=SZ;
      const tctx=tc.getContext('2d');
      // 현재 아이콘 상태
      const tmpImg=new Image(); tmpImg.src=_currentIconUrl();
      await new Promise(r=>{ tmpImg.onload=r; });
      tctx.drawImage(tmpImg,0,0,SZ,SZ);
      // 드로잉 레이어 합성
      tctx.drawImage(dcvs,0,0);
      const b64=tc.toDataURL('image/png').split(',')[1];
      const res=await aiProxy({ body:{ model:'claude-sonnet-4-6', max_tokens:300,
        messages:[{ role:'user', content:[
          {type:'image',source:{type:'base64',media_type:'image/png',data:b64}},
          {type:'text',text:`This SVG icon has ${els.length} path elements (index 0 to ${els.length-1}). The orange painted area shows what the user wants to recolor. Currently detected element indices: ${JSON.stringify([..._hitIdxs])}. What other element indices should also be included to complete the selection of that visual part? Return ONLY a JSON array of additional integer indices. If nothing to add, return [].`}
        ]}]
      }});
      const txt=res.data?.content?.[0]?.text||'[]';
      const added=(JSON.parse(txt.match(/\[[\s\S]*?\]/)?.[0]||'[]')||[]).filter(i=>Number.isInteger(+i)&&+i>=0&&+i<els.length);
      added.forEach(i=>_hitIdxs.add(+i));
      _updateHilite(); _updateUI();
      toast(added.length?`🤖 ${added.length}개 요소 추가됨`:'🤖 추가할 요소 없음');
    }catch(err){ toast('AI 보정 실패: '+(err.message||err)); }
    finally{ aib.disabled=false; aib.textContent='🤖 AI 보정 — 인접 요소 자동 포함'; }
  };
  }catch(err){
    console.error('renderPartsEditor', err);
    prev.innerHTML='<div style="font-size:12px;color:var(--sub);padding:16px;text-align:center">⚠ 오류가 발생했습니다: '+escapeHtml(String(err&&err.message||err))+'</div>';
    list.innerHTML='';
  }
}
function insertIcon(rawSvg, name){
  ...
  const stroke=strokeable?(iconStrokeOf(rawSvg)||2):null;
  const w=120,h=120, c=getViewCenter(w,h), p=page();
  const x=Math.max(0,Math.min(p.w-w,Math.round(c.x))), y=Math.max(0,Math.min(p.h-h,Math.round(c.y)));
  const ne={ id:uid(), type:'image', x, y, w, h, rot:0, fit:'contain', radius:0, clip:'none', borderW:0, borderColor:'#333333',
    _icon:name||'', _iconSvg:rawSvg, _iconColor:color, _iconStroke:stroke, _iconStrokeable:strokeable, _iconMono:iconMono(rawSvg) };
  ne.src=iconSvgToDataUrl(rawSvg, color, strokeable?stroke:null);
  _tabTagNew(ne); page().elements.push(ne); selId=ne.id; selIds=new Set([ne.id]); afterMutate();
}
function updateIconColorSwatch(){ const s=document.getElementById('icon-color-sw'); if(s) s.style.background=_iconColor; }
function openIconModal(){
  // 앱 색상 피커에 아이콘 색 타겟 등록(최초 1회) — CP_TARGETS는 이 시점엔 정의돼 있음
  if(typeof CP_TARGETS!=='undefined' && !CP_TARGETS.iconColor){
    CP_TARGETS.iconColor={ label:'아이콘 색', rich:false, current:()=>_iconColor,
      set:v=>{ _iconColor=(v==='transparent'?'#333333':v); updateIconColorSwatch(); const q=(document.getElementById('icon-search').value||'').trim(); if(_iconResults.length&&q) iconSearch(); } };
  }
  const m=document.getElementById('icon-modal'); if(!m)return; m.style.display='flex'; updateIconColorSwatch();
  setTimeout(()=>{const s=document.getElementById('icon-search'); if(s)s.focus();},0);
}
async function iconSearch(){
  const raw=(document.getElementById('icon-search').value||'').trim();
  const st=document.getElementById('icon-status'), grid=document.getElementById('icon-grid');
  if(!raw){ st.textContent='검색어를 입력하세요 (한글·영어 모두 가능)'; grid.innerHTML=''; return; }
  st.textContent='검색 중…'; grid.innerHTML='';
  const q=await toEnglishQuery(raw);
  const col=String(_iconColor||'#333333').replace('#','%23');
  // 의료 키워드는 전문 컬렉션(healthicons) 우선 — paint 같은 접두어 오염 방지
  const isMedical=ICON_MEDICAL_RAW.has(raw)||/[가-힣]/.test(raw)&&(
    /과$|의학|질환|증상|치료|수술|검사|증후|염증|암$|통$/.test(raw)||
    Object.keys(ICON_KO).filter(k=>raw.includes(k)).some(k=>ICON_MEDICAL_RAW.has(k))
  );
  try{
    let results=[];
    if(isMedical){
      // 1차: healthicons (WHO 의료 전용 컬렉션)
      const r1=await fetch(`https://api.iconify.design/search?query=${encodeURIComponent(q)}&limit=60&prefix=healthicons`);
      const d1=await r1.json(); results=(d1&&d1.icons)||[];
      // 2차: noto 의료 이모지도 추가
      if(results.length<20){
        const r2=await fetch(`https://api.iconify.design/search?query=${encodeURIComponent(q)}&limit=40&prefix=medical-icon`);
        const d2=await r2.json(); const r2list=(d2&&d2.icons)||[];
        const seen=new Set(results); results=[...results,...r2list.filter(n=>!seen.has(n))];
      }
      // 3차: 그래도 적으면 전체 검색으로 보완 (단, 맨 앞 의료 결과 우선 유지)
      if(results.length<10){
        const r3=await fetch(`https://api.iconify.design/search?query=${encodeURIComponent(q)}&limit=96`);
        const d3=await r3.json(); const all=(d3&&d3.icons)||[];
        const seen=new Set(results); results=[...results,...all.filter(n=>!seen.has(n))].slice(0,96);
      }
    } else {
      const r=await fetch(`https://api.iconify.design/search?query=${encodeURIComponent(q)}&limit=96`);
      const d=await r.json(); results=(d&&d.icons)||[];
    }
    _iconResults=results;
    if(!_iconResults.length){ st.textContent=`"${raw}" 결과 없음 — 다른 단어로 검색해보세요`; return; }
    st.textContent=(/[가-힣]/.test(raw)?`${raw} → ${q}${isMedical?' 🏥':''} · `:'')+`${_iconResults.length}개 — 클릭하면 삽입`;
    grid.innerHTML=_iconResults.map(nm=>{ const i=nm.indexOf(':'); const pfx=nm.slice(0,i), ic=nm.slice(i+1);
      const url=`https://api.iconify.design/${pfx}/${ic}.svg?color=${col}&height=44`;
      return `<button class="icon-cell" data-icon="${nm}" title="${nm}" style="aspect-ratio:1;display:flex;align-items:center;justify-content:center;background:var(--panel2);border:1px solid var(--border);border-radius:8px;cursor:pointer;padding:7px"><img src="${url}" style="max-width:100%;max-height:100%" loading="lazy" alt=""></button>`;
    }).join('');
    grid.querySelectorAll('.icon-cell').forEach(b=>b.addEventListener('click',()=>pickIcon(b.dataset.icon)));
  }catch(e){ st.textContent='검색 실패(네트워크): '+(e.message||e); }
}
async function pickIcon(name){
  const i=name.indexOf(':'); const pfx=name.slice(0,i), ic=name.slice(i+1);
  const st=document.getElementById('icon-status'); st.textContent='삽입 중…';
  try{
    // 원본 SVG(currentColor 유지) — 색·굵기는 삽입 후에도 조정할 수 있게 저장
    const r=await fetch(`https://api.iconify.design/${pfx}/${ic}.svg?width=240&height=240`);
    const svg=await r.text();
    if(!/<svg/i.test(svg)) throw new Error('아이콘을 불러오지 못했습니다');
    insertIcon(svg, name);
    document.getElementById('icon-modal').style.display='none';
    toast('아이콘 삽입됨 🔣 — 선택하면 색·굵기 조정 가능');
  }catch(e){ st.textContent='삽입 실패: '+(e.message||e); }
}
// 검색어 자동완성 — 한글은 사전 키, 영어는 Iconify keywords API
let _icSugT=null;
async function iconSuggest(){
  const inp=document.getElementById('icon-search'), box=document.getElementById('icon-suggest');
  if(!inp||!box) return;
  const v=(inp.value||'').trim();
  if(v.length<1){ box.style.display='none'; box.innerHTML=''; return; }
  let items=[];
  if(/[가-힣]/.test(v)){
    const keys=Object.keys(ICON_KO);
    const starts=keys.filter(k=>k.indexOf(v)===0);
    const incl=keys.filter(k=>k.indexOf(v)>0);
    items=[...starts,...incl].slice(0,14).map(k=>({label:`${k} <span style="color:var(--sub);font-size:11px">→ ${ICON_KO[k]}</span>`, value:k}));
  } else {
    try{
      const r=await fetch(`https://api.iconify.design/keywords?prefix=${encodeURIComponent(v)}`);
      const d=await r.json(); const ms=(d&&d.matches)||[];
      items=ms.slice(0,16).map(m=>({label:m, value:m}));
    }catch(_){ items=[]; }
  }
  if(!items.length){ box.style.display='none'; return; }
  box.innerHTML=items.map(it=>`<div class="ic-sug" data-v="${String(it.value).replace(/"/g,'&quot;')}" style="padding:7px 11px;cursor:pointer;font-size:13px;border-bottom:1px solid var(--border)">${it.label}</div>`).join('');
  box.style.display='block';
  box.querySelectorAll('.ic-sug').forEach(d=>{
    d.addEventListener('mouseenter',()=>d.style.background='var(--panel2)');
    d.addEventListener('mouseleave',()=>d.style.background='');
    d.addEventListener('mousedown',ev=>{ ev.preventDefault(); inp.value=d.dataset.v; box.style.display='none'; iconSearch(); });
  });
}
// ── SVG 파일을 "아이콘"으로 가져오기 — 아이콘 모달뿐 아니라 캔버스 드롭/붙여넣기/사진 추가에서도 공용으로 사용 ──
function _isSvgFile(file){ return !!file && (/\.svg$/i.test(file.name||'') || file.type==='image/svg+xml'); }
function _loadSvgText(txt, name){
  const m=String(txt||'').match(/<svg[\s\S]*<\/svg>/i);
  if(!m){ toast('올바른 SVG를 찾지 못했어요'); return; }
  insertIcon(m[0], name||'내 SVG');
  const im2=document.getElementById('icon-modal'); if(im2) im2.style.display='none';
  toast('아이콘 삽입됨 🔣 — 선택하면 색·굵기·부분색칠 조정 가능');
}
function _loadSvgFile(file){
  if(!file) return;
  if(!_isSvgFile(file)){ toast('.svg 파일만 가능해요'); return; }
  const r=new FileReader();
  r.onload=()=>_loadSvgText(r.result, file.name.replace(/\.svg$/i,''));
...
}
{
  ...
  if(si){
    si.addEventListener('keydown',e=>{ if(e.key==='Enter'){ const b=document.getElementById('icon-suggest'); if(b)b.style.display='none'; iconSearch(); } else if(e.key==='Escape'){ const b=document.getElementById('icon-suggest'); if(b)b.style.display='none'; } });
    si.addEventListener('input',()=>{ clearTimeout(_icSugT); _icSugT=setTimeout(iconSuggest,160); });
    si.addEventListener('blur',()=>{ setTimeout(()=>{ const b=document.getElementById('icon-suggest'); if(b)b.style.display='none'; },150); });
  }
  // 부분 색 지정 모달
  const closeIconParts=()=>{ document.getElementById('iconparts-modal').style.display='none'; if(_iconPartsMouseUpHandler){ window.removeEventListener('mouseup',_iconPartsMouseUpHandler); _iconPartsMouseUpHandler=null; } };
  const ipc=document.getElementById('iconparts-close'); if(ipc) ipc.onclick=closeIconParts;
  const ipm=document.getElementById('iconparts-modal'); if(ipm) ipm.addEventListener('mousedown',e=>{ if(e.target.id==='iconparts-modal') closeIconParts(); });
  const ipd=document.getElementById('iconparts-done'); if(ipd) ipd.onclick=closeIconParts;
  const ipr=document.getElementById('iconparts-reset'); if(ipr) ipr.onclick=()=>{ if(_partsEl){ _iconPushUndo(_partsEl); _partsEl._iconParts={}; refreshIconEl(_partsEl); renderCanvas(); save(true); snapshot(); _selPart=null; renderPartsEditor(); } };
  const ipu=document.getElementById('iconparts-undo'); if(ipu) ipu.onclick=()=>{ if(_partsEl) _iconUndo(_partsEl); };

  // ── 내 SVG 파일 가져오기 (파일 선택 · 드래그앤드롭 · 붙여넣기) ──
  const dz=document.getElementById('icon-dropzone'), fi=document.getElementById('icon-file');
  if(dz&&fi){
    dz.addEventListener('click',()=>fi.click());
    fi.addEventListener('change',()=>{ const f=fi.files[0]; fi.value=''; if(f) _loadSvgFile(f); });
    dz.addEventListener('dragover',ev=>{ ev.preventDefault(); dz.style.borderColor='var(--accent)'; dz.style.color='var(--accent)'; });
    dz.addEventListener('dragleave',()=>{ dz.style.borderColor='var(--border)'; dz.style.color='var(--sub)'; });
    dz.addEventListener('drop',ev=>{
      ev.preventDefault(); ev.stopPropagation(); dz.style.borderColor='var(--border)'; dz.style.color='var(--sub)';
      const f=ev.dataTransfer&&ev.dataTransfer.files&&ev.dataTransfer.files[0];
      if(f) _loadSvgFile(f);
      else{ const txt=ev.dataTransfer&&ev.dataTransfer.getData('text'); if(txt) _loadSvgText(txt); }
    });
  }
  // 모달 전체에 드래그앤드롭 허용(그리드 위에 놓아도 동작)
  if(im){
    im.addEventListener('dragover',ev=>{ if(ev.dataTransfer&&[...ev.dataTransfer.types].includes('Files')) ev.preventDefault(); });
    im.addEventListener('drop',ev=>{
      const f=ev.dataTransfer&&ev.dataTransfer.files&&ev.dataTransfer.files[0];
      if(f&&/\.svg$/i.test(f.name)){ ev.preventDefault(); _loadSvgFile(f); }
    });
    im.addEventListener('paste',ev=>{
      if(document.getElementById('icon-modal').style.display==='none') return;
      const cd=ev.clipboardData; if(!cd) return;
      const f=[...(cd.files||[])].find(x=>/\.svg$/i.test(x.name)||x.type==='image/svg+xml');
      if(f){ ev.preventDefault(); _loadSvgFile(f); return; }
      const txt=cd.getData('text'); if(txt&&/<svg[\s\S]*<\/svg>/i.test(txt)){ ev.preventDefault(); _loadSvgText(txt); }
    });
  }
}
...
['stage','canvas'].forEach(id=>{
  ...
  t.addEventListener('drop', ev=>{
    ...
    ev.preventDefault(); ev.stopPropagation(); document.getElementById('stage').classList.remove('drag-over');
    const cRect=canvas.getBoundingClientRect();
    const cx=(ev.clientX-cRect.left)/zoom, cy=(ev.clientY-cRect.top)/zoom;
    const svgs=[...files].filter(f=>_isSvgFile(f));
    const imgs=[...files].filter(f=>/^image\//.test(f.type)&&!_isSvgFile(f));
    const fonts=[...files].filter(f=>!/^image\//.test(f.type)&&/font|\.(ttf|otf|woff2?)$/i.test(f.type+f.name));
    svgs.forEach(f=>_loadSvgFile(f));           // SVG는 아이콘으로(색·굵기·나누기 등 편집 가능)
    imgs.forEach((f,i)=>addImageFromFile(f, cx+i*20, cy+i*20));
    if(fonts.length) importFontFiles(fonts);
  });
});
...
// ── 부분 폰트(리치 텍스트) — 텍스트/도형 안 글자 일부만 다른 폰트로 ──
// run = {text:"조각", font:"폰트명"|null(=기본 폰트)}. 편집(contentEditable) 종료 시 DOM에서 run을 다시 읽어 저장.
function _richEditHtml(runs){
  return runs.map(r=>{
    const t=escapeHtml(r.text||'');
    return r.font ? `<span data-rf-font="${escapeHtml(r.font)}" style="font-family:'${r.font}',sans-serif">${t}</span>` : t;
  }).join('');
}
function _richCollectInline(node, out){
  node.childNodes.forEach(n=>{
    if(n.nodeType===3){ out.push({text:n.textContent, font:null}); }
    else if(n.nodeType===1){
      const tag=n.tagName.toLowerCase();
      if(tag==='br'){ out.push({text:'\n', font:null}); }
      else if(tag==='span' && n.dataset && n.dataset.rfFont){ out.push({text:n.textContent, font:n.dataset.rfFont}); }
      else _richCollectInline(n, out);
    }
  });
}
// contentEditable DOM → run 배열. 브라우저가 Enter를 <div>로 감싸는 경우(줄바꿈)까지 처리.
function _domToRuns(root){
  const out=[];
  const kids=[...root.childNodes];
  const allDiv = kids.length>0 && kids.every(n=>n.nodeType===1 && /^(div|p)$/i.test(n.tagName));
  if(allDiv){ kids.forEach((d,i)=>{ _richCollectInline(d,out); if(i<kids.length-1) out.push({text:'\n',font:null}); }); }
  else _richCollectInline(root, out);
  return out;
}
function _mergeRuns(raw){
  const merged=[];
  raw.forEach(r=>{
    if(!r.text) return;
    const last=merged[merged.length-1], f=r.font||null;
    if(last && (last.font||null)===f) last.text+=r.text; else merged.push({text:r.text, font:f});
  });
  return merged;
}
// 편집 종료 시 호출 — DOM을 파싱해 richField(부분폰트)와 textField(평문, 검색·호환용)를 함께 갱신
function _richFinish(e, inner, textField, richField){
  const runs=_mergeRuns(_domToRuns(inner));
  e[textField]=runs.map(r=>r.text).join('');
  if(runs.length<=1 || runs.every(r=>!r.font)) delete e[richField];
  else e[richField]=runs;
}
// 글머리/번호/강조색 포함 표시용 HTML 생성 (편집 중이 아닐 때)
function setTextInner(inner, e, editing){
  if(editing){
    if(e.richFonts&&e.richFonts.length) inner.innerHTML=_richEditHtml(e.richFonts);
  ...
  if(e.richFonts && e.richFonts.length && !(e.bullet&&e.bullet!=='none')){
    inner.style.whiteSpace='pre-wrap';
    inner.innerHTML = e.richFonts.map(r=>{
      const t=escapeHtml(r.text); const body=hl?wrap(t):t;
      return r.font ? `<span style="font-family:'${r.font}',sans-serif">${body}</span>` : body;
    }).join('');
    return;
  }
...
function renderCanvas(){
  ...
  p.elements.forEach(e=>{ if(_tabHiddenInEdit(e)) return; canvas.appendChild(renderEl(e)); });
  drawSections();
...
function buildHambMenu(){
  ...
  items.forEach((it,idx)=>{
    html+=`<div class="hmx-item" data-idx="${idx}" style="border:1px solid #e6e6ee;border-radius:9px;padding:8px;margin-bottom:7px;background:#fff">
      ...
      ${(it.children||[]).map((k,kidx)=>`
      <div class="hmx-kid" data-idx="${idx}" data-kidx="${kidx}" style="display:flex;gap:5px;align-items:center;margin:6px 0 0 18px">
        <span style="color:#ccc;font-size:11px">↳</span>
        <input class="hmx-kid-name" value="${escapeHtml(k.name||'')}" placeholder="하위 메뉴 이름" style="flex:1;padding:5px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px;color:#222">
        <select class="hmx-kid-link" style="width:96px;padding:5px 6px;border:1px solid #ddd;border-radius:6px;font-size:11px;color:#444">
          ${roots.map(r=>`<option value="${r.id}" ${k.link===r.id?'selected':''}>${escapeHtml(r.name||'페이지')}</option>`).join('')}
        </select>
        <button class="hmx-kid-del" title="삭제" style="border:none;background:none;color:#e05a7a;cursor:pointer;font-size:13px">✕</button>
      </div>`).join('')}
      <button class="hmx-kid-add" data-idx="${idx}" style="margin:6px 0 0 18px;padding:5px 10px;border:1px dashed #c9c9d6;background:#fafaff;border-radius:6px;cursor:pointer;color:#667;font-size:11px">＋ 하위 메뉴 추가</button>
    </div>`;
  });
  ...
  <div style="font-size:11px;font-weight:700;color:#889;margin-bottom:6px">↳ 하위 메뉴 스타일</div>
  <div style="display:flex;gap:8px;align-items:center;font-size:11px;color:#888;margin-bottom:6px">
    <span>글자 크기 <b id="hmx-subsize-val" style="color:#555">${cfg.subSize||Math.max(13,(cfg.itemSize||17)-2)}</b></span>
    <input id="hmx-subsize" type="range" min="11" max="24" value="${cfg.subSize||Math.max(13,(cfg.itemSize||17)-2)}" style="flex:1">
  </div>
  <div style="display:flex;gap:8px;align-items:center;font-size:11px;color:#888">
    <span>굵기</span>
    <select id="hmx-subweight" style="flex:1;padding:5px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px;color:#222">
      <option value="400" ${(cfg.subWeight||500)==400?'selected':''}>보통</option>
      <option value="500" ${(cfg.subWeight||500)==500?'selected':''}>중간</option>
      <option value="700" ${(cfg.subWeight||500)==700?'selected':''}>굵게</option>
      <option value="900" ${(cfg.subWeight||500)==900?'selected':''}>매우굵게</option>
    </select>
    <button type="button" class="panel-cbtn" data-cpkey="hmSubColor" title="하위 메뉴 글자색"><span style="background:${cfg.subColor||cfg.color||'#1a2b5c'}"></span></button>
  </div>`;
  ...
  const ssI=panel.querySelector('#hmx-subsize'); if(ssI){ ssI.addEventListener('input',()=>{ hamburgerCfg().subSize=parseInt(ssI.value)||15; save(true); const sv=panel.querySelector('#hmx-subsize-val'); if(sv)sv.textContent=hamburgerCfg().subSize; }); ssI.addEventListener('change',()=>snapshot()); }
  const swI=panel.querySelector('#hmx-subweight'); if(swI){ swI.addEventListener('change',()=>{ hamburgerCfg().subWeight=parseInt(swI.value)||500; save(true); snapshot(); }); }
  ...
  panel.querySelectorAll('.hmx-kid-name').forEach(inp=>{
    const row=inp.closest('.hmx-kid'), idx=+row.dataset.idx, kidx=+row.dataset.kidx;
    inp.addEventListener('input',e2=>{ hamburgerMaterialize()[idx].children[kidx].name=e2.target.value; save(true); });
    inp.addEventListener('change',snapshot);
  });
  panel.querySelectorAll('.hmx-kid-link').forEach(sel=>{
    const row=sel.closest('.hmx-kid'), idx=+row.dataset.idx, kidx=+row.dataset.kidx;
    sel.addEventListener('change',e2=>{ hamburgerMaterialize()[idx].children[kidx].link=e2.target.value; save(true); snapshot(); });
  });
  panel.querySelectorAll('.hmx-kid-del').forEach(btn=>{
    btn.addEventListener('click',()=>{ const row=btn.closest('.hmx-kid'), idx=+row.dataset.idx, kidx=+row.dataset.kidx; hamburgerMaterialize()[idx].children.splice(kidx,1); save(true); snapshot(); refreshHambMenu(); });
  });
  panel.querySelectorAll('.hmx-kid-add').forEach(btn=>{
    btn.addEventListener('click',()=>{ const idx=+btn.dataset.idx; const it=hamburgerMaterialize()[idx]; if(!it.children) it.children=[]; const r=roots[0]; it.children.push({name:r?r.name:'하위 메뉴', link:r?r.id:''}); save(true); snapshot(); refreshHambMenu(); });
  });
  return panel;
}
...
// ✂️ 영역 잘라내기 — clipCut(요소 박스 기준 0~1 비율 좌표의 닫힌 다각형)이 있으면 CSS clip-path로 그 안쪽을 제거.
// 어떤 타입(텍스트·도형·이미지·표)이든 공통 렌더 지점(elStyleCommon)에서 적용 → 벡터 변환 없이 모든 요소에 동일하게 동작.
function _clipCutCss(e){
  if(!e.clipCut||e.clipCut.length<3) return '';
  const w=e.w,h=e.h;
  const pts=e.clipCut.map(p=>(p.x*w).toFixed(2)+','+(p.y*h).toFixed(2)).join(' L ');
  const d=`M0,0 L${w},0 L${w},${h} L0,${h} Z M${pts} Z`;
...
function elStyleCommon(node,e){
  ...
  node.style.clipPath = _clipCutCss(e);
}
// ── ✂️ 영역 잘라내기 모달 — 텍스트·도형·이미지·표 공통(요소 타입 무관, clip-path만 적용) ──
let _elCutTarget=null, _elCutPts=[];
let _elCutShape='poly', _elCutDragStart=null; // 'poly'=점찍기, 'rect'=사각형(드래그), 'ellipse'=원형(드래그)
let _elCutMouseUpHandler=null; // window 리스너 — 재렌더마다 이전 것 제거(누수 방지)
function openElementCut(e){
  if(!e) return;
  _elCutTarget=e; _elCutPts=[]; _elCutShape='poly'; _elCutDragStart=null;
  document.getElementById('elcut-modal').style.display='flex';
  renderElCutEditor();
}
function _elCutHintText(){
  if(_elCutShape==='rect') return '▭ 드래그로 사각형을 그리세요 — 손을 떼면 바로 잘려요';
  if(_elCutShape==='ellipse') return '⭕ 드래그로 원형을 그리세요 — 손을 떼면 바로 잘려요';
  return '지울 부분을 <b>클릭해서 점을 찍어</b> 영역을 감싸세요. 더블클릭 또는 완료 버튼으로 확정합니다.';
}
let _elCutScale=1;
function renderElCutEditor(){
  const e=_elCutTarget; if(!e) return;
  const hint=document.getElementById('elcut-hint'); if(hint) hint.innerHTML=_elCutHintText();
  const prev=document.getElementById('elcut-preview');
  const MAXP=280;
  const scale=Math.min(1, MAXP/Math.max(e.w,e.h));
  _elCutScale=scale;
  const pw=Math.round(e.w*scale), ph=Math.round(e.h*scale);
  prev.style.width=pw+'px'; prev.style.height=ph+'px'; prev.style.overflow='hidden';
  prev.innerHTML='';
  const holder=document.createElement('div');
  holder.style.cssText=`position:absolute;top:0;left:0;width:${e.w}px;height:${e.h}px;transform:scale(${scale});transform-origin:top left`;
  const clone=renderEl({...e, clipCut:null, id:'_ec'+e.id}); // 원본 그대로(기존 잘림 제외) 보고 새 영역을 그리게
  clone.style.position='absolute'; clone.style.left='0'; clone.style.top='0'; clone.style.transform='none';
  clone.classList.remove('selected','multi-sel');
  holder.appendChild(clone);
  prev.appendChild(holder);
  const cvs=document.createElement('canvas'); cvs.width=pw; cvs.height=ph; cvs.id='elcut-draw';
  cvs.style.cssText='position:absolute;inset:0;cursor:crosshair';
  prev.appendChild(cvs);
  const ctx=cvs.getContext('2d');
  function redraw(){
    ctx.clearRect(0,0,pw,ph);
    ctx.strokeStyle='rgba(255,0,150,0.95)'; ctx.lineWidth=2; ctx.lineJoin='round'; ctx.lineCap='round';
    if(_elCutPts.length>1){ ctx.beginPath(); ctx.moveTo(_elCutPts[0].x,_elCutPts[0].y); for(let i=1;i<_elCutPts.length;i++) ctx.lineTo(_elCutPts[i].x,_elCutPts[i].y); ctx.stroke(); }
    if(_elCutShape==='poly'){ ctx.fillStyle='rgba(255,0,150,0.95)'; _elCutPts.forEach(p=>{ ctx.beginPath(); ctx.arc(p.x,p.y,3.5,0,Math.PI*2); ctx.fill(); }); }
  }
  const MIN_DRAG=6;
  cvs.onmousedown=ev=>{
    ev.preventDefault();
    const r=cvs.getBoundingClientRect(), x=ev.clientX-r.left, y=ev.clientY-r.top;
    if(_elCutShape==='poly'){ _elCutPts.push({x,y}); redraw(); _updateElCutBtns(); return; }
    _elCutDragStart={x,y}; _elCutPts=[{x,y}]; redraw();
  };
  cvs.onmousemove=ev=>{
    if(_elCutShape==='poly'||!_elCutDragStart) return;
    const r=cvs.getBoundingClientRect(), x=ev.clientX-r.left, y=ev.clientY-r.top;
    _elCutPts=_elCutShape==='rect'?_rectPts(_elCutDragStart,{x,y}):_ellipsePts(_elCutDragStart,{x,y},40);
    redraw();
  };
  if(_elCutMouseUpHandler) window.removeEventListener('mouseup', _elCutMouseUpHandler);
  _elCutMouseUpHandler=ev=>{
    if(_elCutShape==='poly'||!_elCutDragStart) return;
    const r=cvs.getBoundingClientRect();
    const ex=ev.clientX-r.left, ey=ev.clientY-r.top;
    const st=_elCutDragStart; _elCutDragStart=null;
    if(Math.hypot(ex-st.x,ey-st.y)>=MIN_DRAG) _applyElCut(); else { _elCutPts=[]; redraw(); }
  };
  window.addEventListener('mouseup', _elCutMouseUpHandler);
  cvs.ondblclick=ev=>{ if(_elCutShape==='poly'&&_elCutPts.length>=3){ ev.preventDefault(); _applyElCut(); } };
  _updateElCutBtns();
}
function _updateElCutBtns(){
  const isPoly=_elCutShape==='poly';
  const done=document.getElementById('elcut-done'); if(done) done.style.display=(isPoly&&_elCutPts.length>=3)?'inline-block':'none';
  const cancel=document.getElementById('elcut-cancelpts'); if(cancel) cancel.style.display=(isPoly&&_elCutPts.length>0)?'inline-block':'none';
}
function _applyElCut(){
  const e=_elCutTarget; if(!e||_elCutPts.length<3) return;
  e.clipCut=_elCutPts.map(p=>({x:(p.x/_elCutScale)/e.w, y:(p.y/_elCutScale)/e.h}));
  document.getElementById('elcut-modal').style.display='none';
  afterMutate();
  toast('✂️ 영역이 잘렸어요');
}
{
  const closeElCut=()=>{ document.getElementById('elcut-modal').style.display='none'; if(_elCutMouseUpHandler){ window.removeEventListener('mouseup',_elCutMouseUpHandler); _elCutMouseUpHandler=null; } };
  const ecc=document.getElementById('elcut-close'); if(ecc) ecc.onclick=closeElCut;
  const eco=document.getElementById('elcut-ok'); if(eco) eco.onclick=closeElCut;
  const em=document.getElementById('elcut-modal'); if(em) em.addEventListener('mousedown',ev=>{ if(ev.target.id==='elcut-modal') closeElCut(); });
  const ecd=document.getElementById('elcut-done'); if(ecd) ecd.onclick=()=>_applyElCut();
  const ecx=document.getElementById('elcut-cancelpts'); if(ecx) ecx.onclick=()=>{ _elCutPts=[]; renderElCutEditor(); };
  const ecr=document.getElementById('elcut-reset'); if(ecr) ecr.onclick=()=>{ if(_elCutTarget){ delete _elCutTarget.clipCut; afterMutate(); toast('원본으로 되돌렸어요'); renderElCutEditor(); } };
  const ecsPoly=document.getElementById('elcut-cs-poly'), ecsRect=document.getElementById('elcut-cs-rect'), ecsEllipse=document.getElementById('elcut-cs-ellipse');
  function setElCutShape(s){
    _elCutShape=s; _elCutPts=[]; _elCutDragStart=null;
    if(ecsPoly) ecsPoly.classList.toggle('on', s==='poly');
    if(ecsRect) ecsRect.classList.toggle('on', s==='rect');
    if(ecsEllipse) ecsEllipse.classList.toggle('on', s==='ellipse');
    renderElCutEditor();
  }
  if(ecsPoly) ecsPoly.onclick=()=>setElCutShape('poly');
  if(ecsRect) ecsRect.onclick=()=>setElCutShape('rect');
  if(ecsEllipse) ecsEllipse.onclick=()=>setElCutShape('ellipse');
}
function renderEl(e){
  ...
  if(e.type==='text'){
   ...
   if(e.shape==='line'||e.shape==='line-arrow'){
    ...
    if(e.stRichFonts&&e.stRichFonts.length) inner.innerHTML=_richEditHtml(e.stRichFonts);
    else inner.textContent=e.stext||'';
  ...
  if(e.type==='table'){
    ...
    // 둥근 모서리(radius>0): 래퍼가 실제 둥근 CSS 테두리를 그려 곡선을 완벽히 따름.
    // 이때 셀 바깥쪽(맨 위/아래/좌/우) 테두리는 렌더 시 제거 → 직선 모서리가 잘리지 않음.
    const _bw=(e.borderW!=null?e.borderW:1), _bc=e.borderColor||'#333', rounded=e.radius>0;
    const wrap=document.createElement('div');
    wrap.style.cssText=`position:absolute;inset:0;overflow:hidden;box-sizing:border-box${rounded?`;border-radius:${e.radius}px${_bw>0?`;border:${_bw}px solid ${_bc}`:''}`:''}`;
  ...
  node.addEventListener('contextmenu', ev=>{ ev.preventDefault(); if(_linkPick) return; if(!selIds.has(e.id)){ if(e.groupId) selIds=new Set(page().elements.filter(x=>x.groupId===e.groupId).map(x=>x.id)); else selIds=new Set([e.id]); } selId=e.id; renderCanvas(); renderProps(); if(e.type==='table') showTableCtx(ev.clientX, ev.clientY, e, ev); else showElementCtx(ev.clientX, ev.clientY, e); });
  if(_linkPick){
    if(e.id===_linkPick.srcId) node.classList.add('lp-src');
    else node.classList.add(lpIsLinked(e)?'lp-linked':'lp-cand');
  }
  return node;
}
...
function startGroupResize(ev,pos,bb){
  ev.preventDefault();
  const arr=selAll().concat(twinMates(selIds)); const t=_snapTargets(); // 숨은 쌍둥이도 같은 비율로 크기조절
...
function startDrag(ev,e){
  ...
  twinMates(selIds).forEach(e2=>{ if(!init[e2.id]) init[e2.id]={ox:e2.x,oy:e2.y}; }); // 숨은 쌍둥이도 함께 이동
...
function startResize(ev,e,pos){
  ...
  rez={ id:e.id, pos, sx:ev.clientX, sy:ev.clientY, ox:e.x, oy:e.y, ow:e.w, oh:e.h, snapX:t.sxT, snapY:t.syT,
    twins: twinMates(new Set([e.id])).map(t2=>({e:t2,ox:t2.x,oy:t2.y,ow:t2.w,oh:t2.h})) };
...
function onResize(ev){
  ...
  if(rez.twins&&rez.twins.length){ const sx2=e.w/rez.ow, sy2=e.h/rez.oh; rez.twins.forEach(t2=>{ t2.e.x=Math.round(e.x+(t2.ox-rez.ox)*sx2); t2.e.y=Math.round(e.y+(t2.oy-rez.oy)*sy2); t2.e.w=Math.max(8,Math.round(t2.ow*sx2)); t2.e.h=Math.max(8,Math.round(t2.oh*sy2)); const n2=canvas.querySelector(`[data-id="${t2.e.id}"]`); if(n2) elStyleCommon(n2,t2.e); }); }
  showGuides(gx,gy);
  syncPosInputs(e);
}
...
// ───────────────────────── 텍스트 인라인 편집 ─────────────────────────
let _editingCtx=null; // {e, inner} — 지금 편집 중인 요소/DOM (폰트 피커가 "부분 선택 적용"을 판단하는 데 사용)
function startEdit(node,e){
  node.classList.add('editing');
  const inner=node.querySelector('.inner');
  const field = e.type==='shape' ? 'stext' : 'text';
  const richField = e.type==='shape' ? 'stRichFonts' : 'richFonts';
  if(e.type==='shape'){
    if(inner.parentElement) inner.parentElement.style.pointerEvents='auto';
    if(e.stRichFonts&&e.stRichFonts.length) inner.innerHTML=_richEditHtml(e.stRichFonts);
    else inner.textContent=e.stext||'';
  }
  else setTextInner(inner, e, true); // 편집 중엔 원문(글머리/강조 제거, 부분폰트는 유지)
  inner.style.whiteSpace='pre-wrap';
  ...
  const finish=()=>{
    ...
    _richFinish(e, inner, field, richField);
    if(_editingCtx&&_editingCtx.e===e) _editingCtx=null;
    inner.removeEventListener('blur',finish);
    afterMutate();
  };
  inner.addEventListener('blur',finish);
}
// 지금 편집 중인 텍스트 안에서 "글자가 실제로 선택돼 있는지" — 있으면 부분 폰트 적용 대상
function _activeRichSelection(){
  if(!_editingCtx) return null;
  const sel=window.getSelection(); if(!sel||sel.rangeCount===0||sel.isCollapsed) return null;
  const r=sel.getRangeAt(0);
  if(!_editingCtx.inner.contains(r.commonAncestorContainer)) return null;
  return r;
}
// 선택 영역을 <span data-rf-font>로 감싸 즉시 그 부분만 폰트 적용(편집 상태 유지, blur 시 richFonts로 확정)
function applyFontToSelection(fontFamily){
  const r=_activeRichSelection(); if(!r) return false;
  const span=document.createElement('span');
  span.dataset.rfFont=fontFamily; span.style.fontFamily=`'${fontFamily}',sans-serif`;
  try{ span.appendChild(r.extractContents()); r.insertNode(span); }catch(_){ return false; }
...
canvas.addEventListener('mousedown', ev=>{
  ...
  function onMU(e){
    ...
    for(const e2 of page().elements){
      if(_tabHiddenInEdit(e2)) continue; // 편집 뷰에서 숨긴 다른 탭 요소는 선택 제외
...
// ───────────────────────── 렌더: 속성 패널 ─────────────────────────
// 풀블리드 토글 — 도형/이미지(섹션 배경)를 발행 시 화면 끝까지 늘림
function bleedToggleHtml(e){
  return `<div class="grp" style="border-top:1px solid var(--border);padding-top:10px"><label style="display:flex;align-items:center;gap:7px;cursor:pointer;text-transform:none;margin:0"><input type="checkbox" id="el-bleed" ${e.bleed?'checked':''} style="width:auto"> 🖼 화면 끝까지 (풀블리드 배경)</label><div style="font-size:11px;color:var(--sub);margin-top:4px">발행하면 화면 좌우 끝까지 늘어납니다. 섹션 배경 사진·색에 사용 (글자·카드 위에 따로 올리세요).</div></div>`;
}
function wireBleedToggle(e){
  const cb=document.getElementById('el-bleed');
  if(cb) cb.addEventListener('change',()=>{ if(cb.checked) e.bleed=true; else delete e.bleed; afterMutate(); });
}
function renderProps(){
  ...
  if(e.type==='text'){
    ...
    // 아이콘(Iconify)이면 색·선굵기 조정 컨트롤
    if(e._icon){
      html += `<div class="grp" style="border:1px solid var(--border);border-radius:9px;padding:10px 11px;background:var(--panel2)"><label>🔣 아이콘</label>`;
      if(e._iconMono!==false){
        html += `<div style="margin-bottom:8px"><button type="button" class="panel-cbtn" data-cpkey="iconElColor" title="아이콘 색" style="width:100%;justify-content:flex-start"><span style="background:${e._iconColor||'#333333'}"></span><span style="font-size:12px;color:var(--sub)">색 변경</span></button></div>`;
      } else {
        html += `<div style="font-size:11px;color:var(--sub);margin-bottom:8px">여러 색으로 고정된 아이콘이라 색 변경은 안 돼요.</div>`;
      }
      // 두께 — 모든 아이콘에 균일 적용(fill·stroke 함께). 0=원본
      const _th=(e._iconThick!=null?e._iconThick:0);
      html += `<div><label style="text-transform:none">굵기 <span id="ic-stroke-v" style="float:right;color:var(--accent)">${_th>0?'+':''}${_th}</span></label><input type="range" id="ic-stroke" min="-3" max="6" step="0.25" value="${_th}" style="width:100%"></div>`;
      html += `<div style="font-size:10px;color:var(--sub);margin-top:2px">0=원본 · ◀얇게 / 두껍게▶ (선·면 함께 변함)</div>`;
      const _pc=iconPartCount(e._iconSvg||'');
      if(_pc>=2) html += `<div style="margin-top:9px"><button type="button" class="tb-btn" id="ic-parts" style="width:100%">🎨 부분 색칠 (칠해서 선택)</button></div>`;
      html += `</div>`;
    }
    html += `<div class="grp"><label>이미지</label><button class="tb-btn" id="i-replace" style="width:100%">🔄 ${e._icon?'다른 그림으로 교체':'사진 교체'}</button></div>`;
    if(!e._icon){   // 아이콘엔 이미지 상자용 컨트롤(테두리·채움·액자·모서리·풀블리드) 불필요 — 선 굵기는 위 🔣 아이콘에서
    ...
    html += `<div class="sec-hd">▾ 테두리 · 모서리</div>`;
    html += `<div class="grp"><button id="tb-border-dlg" style="width:100%;display:flex;align-items:center;justify-content:center;gap:7px;padding:9px;border:1px solid var(--accent);border-radius:8px;background:transparent;color:var(--accent);cursor:pointer;font-weight:700;font-size:12px">⊞ 테두리 / 모서리 둥글기 설정…</button></div>`;
    html += `<div class="grp"><div style="font-size:11px;color:var(--sub)">선 두께·색·스타일·대각선·모서리 둥글기는 위 [테두리 설정] 창에서 조절합니다.</div></div>`;
...
function bindProps(e){
  ...
  if(e.type==='text'){
    ...
    function renderFontList(q=''){
      ...
      fpList.querySelectorAll('.fitem').forEach(item=>{
        item.addEventListener('mousedown', ev=>{ if(ev.target.dataset.delfont) return; if(_activeRichSelection()) ev.preventDefault(); }); // 편집 중 선택이 blur로 날아가지 않게
        item.addEventListener('click',(ev)=>{
          if(ev.target.dataset.delfont) return;
          const fam=item.dataset.fam;
          if(applyFontToSelection(fam)){
            fpDd.style.display='none'; fpBtn.classList.remove('open');
            toast(`선택한 글자에 "${fam}" 폰트 적용됨`);
            return; // 부분 선택만 적용 — 요소 전체 폰트는 그대로 둠
          }
    ...
    fpBtn.addEventListener('mousedown', ev=>{ if(_activeRichSelection()) ev.preventDefault(); }); // 편집 중 선택 유지
    fpBtn.addEventListener('click',(ev)=>{
      ev.stopPropagation();
      const hadSel=!!_activeRichSelection();
      ...
      renderFontList();
      if(!hadSel) setTimeout(()=>$('font-search').focus(),0); // 편집 중 선택이 있으면 검색창 포커스로 선택이 풀리지 않게 생략
    });
    ...
    $('font-file-btn').addEventListener('click',ev=>{ ev.stopPropagation(); fontFileInput.onchange=()=>{ const fs=fontFileInput.files; fontFileInput.value=''; if(fs&&fs.length) importFontFiles(fs,()=>{ fpDd.style.display='none'; fpBtn.classList.remove('open'); }); }; fontFileInput.click(); });
    // 폰트 드롭다운에 직접 드래그앤드롭으로 폰트 파일 넣기
    { const dd=$('font-picker-dd');
      if(dd){ dd.addEventListener('dragover',ev=>{ if(ev.dataTransfer&&[...ev.dataTransfer.types].includes('Files')){ ev.preventDefault(); ev.stopPropagation(); dd.style.outline='2px dashed var(--accent)'; } });
        dd.addEventListener('dragleave',ev=>{ dd.style.outline=''; });
        dd.addEventListener('drop',ev=>{ const fs=ev.dataTransfer&&ev.dataTransfer.files; if(!fs||!fs.length) return; ev.preventDefault(); ev.stopPropagation(); dd.style.outline=''; importFontFiles(fs); }); } }
    ...
    // 아이콘 색·굵기
    if(e._icon){
      if(typeof CP_TARGETS!=='undefined' && !CP_TARGETS.iconElColor){
        CP_TARGETS.iconElColor={ label:'아이콘 색', rich:false,
          current:()=>{ const x=selId?el(selId):null; return (x&&x._iconColor)||'#333333'; },
          set:v=>{ const x=selId?el(selId):null; if(x&&x._iconSvg){ x._iconColor=(v==='transparent'?'#333333':v); refreshIconEl(x); renderCanvas(); save(true); } } };
      }
      const isw=$('ic-stroke');
    ...
    if($('tb-border-dlg')) $('tb-border-dlg').addEventListener('click',()=>showBorderDialog(e));
  ...
  $('el-del').addEventListener('click',()=>{ const del=expandTabDelete(new Set([e.id])); page().elements=page().elements.filter(x=>!del.has(x.id)); selId=null; selIds=new Set(); afterMutate(); });
...
// ── 캔버스 직접 연결 모드 ── 트리거 효과 선택 후 캔버스에서 좌클릭으로 대상을 바로 연결
let _linkPick=null; // {srcId, partnerType, group, idx, label}
function lpIsLinked(t){ const lp=_linkPick; if(!lp) return false; const xfx=t.fx||{}; return xfx.type===lp.partnerType && xfx.group===lp.group && (lp.idx===undefined||(xfx.idx||0)===lp.idx); }
function startLinkPick(srcEl, partnerType, group, idx, label){
  _linkPick={srcId:srcEl.id, partnerType, group, idx, label:label||'대상'};
  document.body.classList.add('lp-on'); showLinkPickBar(); renderCanvas(); renderProps();
}
function endLinkPick(){
  if(!_linkPick) return;
  _linkPick=null; document.body.classList.remove('lp-on');
  const bar=document.getElementById('lp-bar'); if(bar) bar.remove();
  renderCanvas(); renderProps();
}
function lpToggle(t){
  const lp=_linkPick; if(!lp||t.id===lp.srcId) return;
  if(lpIsLinked(t)){ delete t.fx; toast('연결 해제됨'); }
  else { t.fx={type:lp.partnerType, group:lp.group}; if(lp.idx!==undefined) t.fx.idx=lp.idx; toast('연결됨 ✓'); }
  renderCanvas(); save(true); snapshot(); showLinkPickBar();
}
function showLinkPickBar(){
  const lp=_linkPick; if(!lp) return;
  let bar=document.getElementById('lp-bar');
  if(!bar){ bar=document.createElement('div'); bar.id='lp-bar'; document.body.appendChild(bar); makeBarDraggable(bar); }
  const cnt=page().elements.filter(x=>x.id!==lp.srcId && lpIsLinked(x)).length;
  bar.innerHTML=`<span>🎯 연결할 <b>${lp.label}</b>을(를) 캔버스에서 <b>클릭</b>하세요 · 다시 클릭=해제 · 줌으로 화면 크기조절 가능 · 현재 <b>${cnt}개</b> 연결됨</span><button id="lp-done">✓ 완료</button>`;
  bar.querySelector('#lp-done').onclick=endLinkPick;
}
document.addEventListener('keydown',ev=>{ if(ev.key==='Escape' && _linkPick){ ev.stopPropagation(); endLinkPick(); } }, true);
// ── 탭 섹션 편집 모드 ── 한 탭의 내용만 캔버스에 보이게 + 새 요소 자동 소속 + 선택 일괄 지정
let _tabEditView=null; // {group, idx}
// 편집 중인 탭=그 탭 활성, 기본 화면(전체보기)=아무 탭도 안 눌린 중립(선택 전만). twin 기준이라 위치/크기 바뀌어도 안 꼬임.
function _tabVisibleIdx(group){ return (_tabEditView && _tabEditView.group===group) ? _tabEditView.idx : -1; }
function _tabHiddenInEdit(e){
  const f=e.fx; if(!f || f.type!=='tab-content') return false;
  const vis=_tabVisibleIdx(f.group);
  if(vis>=0) return (f.idx||0)!==vis;                 // 탭별 편집: 그 탭(선택 후 + 내용)만 보임
  if(e.twin && /_b\d+$/.test(e.twin)) return true;    // 전체보기: '선택 후 버튼'은 숨김 → 선택 전(회색)이 보임
  return (f.idx||0)!==0;                              // 내용 패널은 탭 0만 표시
}
function _tabTagNew(e){ if(_tabEditView){ e.fx={type:'tab-content', group:_tabEditView.group, idx:_tabEditView.idx}; } return e; }
function tabGroupIdxs(group){ const s=new Set(); page().elements.forEach(e=>{ const f=e.fx; if(f && (f.type==='tab-trigger'||f.type==='tab-content') && f.group===group) s.add(f.idx||0); }); return [...s].sort((a,b)=>a-b); }
// 그룹 G의 트리거(선택 전 버튼) 행 기준 y. 이보다 충분히 아래면 '내용 패널', 아니면 '버튼'으로 구분(크기 무관).
function _tabTrigOf(G,triggers){ return triggers.find(t=>t.fx.group===G)||triggers[0]; }
function _isTabPanel(o,tr){ return (o.y - tr.y) > tr.h*1.5; }
// 선택 후(파랑) 버튼의 '위치'를 선택 전(회색) 트리거에 맞춤 — 기존에 어긋난 탭 정렬용(크기는 유지)
function _snapTabSelButtons(){
  const els=page().elements;
  const triggers=els.filter(o=>o.fx&&o.fx.type==='tab-trigger');
  triggers.forEach(tr=>{
    const g=tr.fx.group, idx=tr.fx.idx||0;
    els.forEach(o=>{ const f=o.fx; if(!f||f.type!=='tab-content'||f.group!==g||(f.idx||0)!==idx) return;
      if(!_isTabPanel(o,tr)){ o.x=tr.x; o.y=tr.y; }  // 버튼 행에 있는 선택후 버튼만 위치 정렬(크기 유지)
    });
  });
}
// 탭 요소를 옮기면 짝(선택전↔선택후 버튼은 같은 idx끼리, 내용 패널은 모두)을 '같은 거리만큼' 따라 이동 — 위치만, 크기는 보존
function _syncTabMove(){
  if(!drag) return;
  const els=page().elements;
  const triggers=els.filter(o=>o.fx&&o.fx.type==='tab-trigger'); if(!triggers.length) return;
  const movedGroups=new Set(); [...selIds].forEach(id=>{ const e2=el(id); if(e2&&e2.groupId) movedGroups.add(e2.groupId); });
  const done=new Set();
  movedGroups.forEach(gid=>{
    const sib=els.find(o=>o.groupId===gid && o.fx && (o.fx.type==='tab-trigger'||o.fx.type==='tab-content')); if(!sib) return;
    const ref=els.find(o=>o.groupId===gid && drag.els[o.id]); if(!ref) return;
    const dx=ref.x-drag.els[ref.id].ox, dy=ref.y-drag.els[ref.id].oy; if(!dx&&!dy) return;
    const f=sib.fx, G=f.group, idx=f.idx||0, tr=_tabTrigOf(G,triggers);
    const movedIsPanel=(f.type==='tab-content') && _isTabPanel(sib,tr);
    const targets=new Set();
    if(movedIsPanel){
      // 내용 패널 이동 → 같은 그룹의 모든 내용 패널 따라오기 (크기 무관, y 위치로 판별)
      els.forEach(o=>{ if(o.fx&&o.fx.type==='tab-content'&&o.fx.group===G&&_isTabPanel(o,tr)) targets.add(o.groupId); });
    } else {
      // 버튼 이동(선택전 트리거 or 선택후) → 같은 idx의 버튼 행 요소들 따라오기
      els.forEach(o=>{ if(o.fx&&(o.fx.type==='tab-trigger'||o.fx.type==='tab-content')&&o.fx.group===G&&(o.fx.idx||0)===idx&&!_isTabPanel(o,tr)) targets.add(o.groupId); });
    }
    targets.forEach(tgid=>{ if(tgid===gid||movedGroups.has(tgid)||done.has(tgid)) return; done.add(tgid); els.forEach(o=>{ if(o.groupId===tgid){ o.x=Math.round(o.x+dx); o.y=Math.round(o.y+dy); } }); });
  });
}
function setTabEditView(group, idx){ _tabEditView={group, idx}; document.body.classList.add('tev-on'); showTabEditBar(); renderCanvas(); renderProps(); renderFxPanel(); }
function clearTabEditView(){ if(!_tabEditView) return; _tabEditView=null; document.body.classList.remove('tev-on'); const b=document.getElementById('tev-bar'); if(b) b.remove(); renderCanvas(); renderProps(); renderFxPanel(); }
function tabAssignSelection(){ const v=_tabEditView; if(!v) return; const arr=selAll(); if(!arr.length){ toast('먼저 캔버스에서 요소를 선택하세요'); return; } arr.forEach(e=>{ e.fx={type:'tab-content', group:v.group, idx:v.idx}; }); afterMutate(); toast(`${arr.length}개를 '탭 ${v.idx}' 내용으로 지정`); showTabEditBar(); }
// 상단 배너(탭편집/연결)를 드래그로 옮길 수 있게 — 버튼 클릭은 그대로
function makeBarDraggable(bar){
  bar.style.cursor='move';
  bar.addEventListener('mousedown',function(ev){
    ev.stopPropagation();
    if(ev.target.closest('button')) return; // 버튼은 클릭 동작 유지
    ev.preventDefault();
    const r=bar.getBoundingClientRect();
    bar.style.left=r.left+'px'; bar.style.top=r.top+'px'; bar.style.transform='none';
    const ox=ev.clientX-r.left, oy=ev.clientY-r.top;
    function mm(e){ bar.style.left=Math.max(0,Math.min(window.innerWidth-r.width, e.clientX-ox))+'px'; bar.style.top=Math.max(0,Math.min(window.innerHeight-r.height, e.clientY-oy))+'px'; }
    function mu(){ window.removeEventListener('mousemove',mm); window.removeEventListener('mouseup',mu); }
    window.addEventListener('mousemove',mm); window.addEventListener('mouseup',mu);
  });
}
function showTabEditBar(){
  const v=_tabEditView; if(!v) return;
  let b=document.getElementById('tev-bar');
  if(!b){ b=document.createElement('div'); b.id='tev-bar'; document.body.appendChild(b); makeBarDraggable(b); }
  const cnt=page().elements.filter(e=>{const f=e.fx;return f&&f.type==='tab-content'&&f.group===v.group&&(f.idx||0)===v.idx;}).length;
  const tabs=tabGroupIdxs(v.group).map(i=>`<button class="tev-bar-tab${i===v.idx?' on':''}" data-i="${i}">탭 ${i}</button>`).join('');
  b.innerHTML=`<span>📑 편집 중 · 새 요소가 <b>탭 ${v.idx}</b>에 들어감 · ${cnt}개</span><span class="tev-bar-tabs">${tabs}</span><button id="tev-assign">선택 요소 지정</button><button id="tev-done">✓ 완료</button>`;
  b.querySelectorAll('.tev-bar-tab').forEach(btn=>btn.onclick=()=>setTabEditView(v.group, +btn.dataset.i));
...
function renderFxPanel(){
  ...
  if(ft && _fxPending===null){
    ...
    if(ft==='hover-show'||ft==='hover-hide'){
      ...
      opts+=`<div class="grp"><label class="lbl">${partnerLabel}</label>
        <button class="tb-btn primary lp-pick-btn" data-pt="${partnerType}" data-grp="${myGroup}" data-lbl="${partnerLabel}" style="width:100%;margin-bottom:7px">🎯 화면에서 직접 연결하기</button>
        <div style="font-size:11px;color:var(--sub);margin-bottom:6px">버튼을 누르고 캔버스에서 대상을 클릭하세요 · 아래 목록에서 골라도 됩니다</div>
    ...
    if(ft==='tab-trigger'||ft==='tab-content'){
      ...
      <div class="grp"><label class="lbl">📑 탭별 편집 <span style="color:var(--sub);font-weight:400">(한 탭씩 보면서 편집)</span></label>
        <div style="display:flex;flex-wrap:wrap;gap:5px">
          ${tabGroupIdxs(myGroup).map(i=>`<button class="tb-btn tev-btn${_tabEditView&&_tabEditView.group===myGroup&&_tabEditView.idx===i?' on':''}" data-grp="${myGroup}" data-i="${i}">탭 ${i}</button>`).join('')}
          <button class="tb-btn tev-all">전체 보기</button>
        </div>
        <div style="font-size:11px;color:var(--sub);margin-top:6px">탭을 누르면 그 탭 내용만 캔버스에 보이고, 새로 추가하는 요소는 그 탭에 자동 소속됩니다.</div>
      </div>
      ${ft==='tab-trigger'?`<div class="grp" style="background:rgba(108,123,255,.08);border:1px solid var(--border);border-radius:8px;padding:9px 11px">
        <div style="font-size:12px;color:var(--text);font-weight:700;margin-bottom:3px">🎨 선택 전 / 선택 후 둘 다 디자인</div>
        <div style="font-size:11px;color:var(--sub)">이 <b>회색 버튼 = 선택 전</b> 모양이고, 같은 자리에 겹친 <b>파란 버튼 = 선택 후</b> 모양입니다. 둘 다 캔버스에서 색·글자를 자유롭게 바꾸세요. 「📑 탭별 편집」에서 탭을 고르면 그 탭의 '선택 후' 버튼이 보여 편집할 수 있습니다.</div>
      </div>`:''}
      <div class="grp"><label class="lbl">${partnerLabel}</label>
        <button class="tb-btn primary lp-pick-btn" data-pt="${partnerType}" data-grp="${myGroup}" data-idx="${myIdx}" data-lbl="${partnerLabel}" style="width:100%;margin-bottom:7px">🎯 화면에서 직접 연결하기</button>
        <div style="font-size:11px;color:var(--sub);margin-bottom:6px">버튼을 누르고 캔버스에서 이 탭 번호의 내용을 클릭하세요 · 아래 목록에서 골라도 됩니다</div>
        <div style="display:flex;flex-direction:column;gap:5px;max-height:160px;overflow-y:auto" id="el-linker">
        ${pageEls.length===0?`<div style="font-size:12px;color:var(--sub);text-align:center;padding:10px">페이지에 다른 요소가 없습니다</div>`
        :pageEls.map(x=>{
          const xfx=x.fx||{};
    ...
    if(ft==='slider'){
      ...
      <div class="btn-grp grp"><button id="eff-peek" class="${fx.peek?'on':''}" style="width:100%">🖼 양옆 미리보기 (센터 모드)</button></div>
      <div style="font-size:11px;color:var(--sub);margin-top:-4px">켜면 가운데 사진이 크게, 좌우 사진이 살짝 보입니다 (발행/미리보기에서 확인)</div>`;
    }
    if(ft==='hover-expand'){
  ...
  // 📑 탭별 편집 스위처
  box.querySelectorAll('.tev-btn').forEach(b=>b.addEventListener('click',()=>setTabEditView(b.dataset.grp, +b.dataset.i)));
  box.querySelectorAll('.tev-all').forEach(b=>b.addEventListener('click',clearTabEditView));
  // 🎯 화면에서 직접 연결 버튼 → 캔버스 연결 모드 진입
  box.querySelectorAll('.lp-pick-btn').forEach(b=>{
    b.addEventListener('click',()=>{
      const idx = b.dataset.idx!==undefined ? +b.dataset.idx : undefined;
      startLinkPick(e, b.dataset.pt, b.dataset.grp, idx, b.dataset.lbl);
    });
  });
  // 요소 연결/해제 클릭
  ...
  { const bp=document.getElementById('eff-peek'); if(bp) bp.addEventListener('click',()=>{ if(!e.fx)e.fx={}; e.fx.peek=!e.fx.peek; renderFxPanel(); save(true); snapshot(); }); }
}
...
// ── 우클릭 링크 메뉴 ──
// z순서(레이어) 변경 — 선택 요소들을 맨앞/앞으로/뒤로/맨뒤로
function _zorder(dir){
  const arr=page().elements, sel=selAll(); if(!sel.length) return;
  const ids=new Set(sel.map(e=>e.id));
  if(dir==='front'){ const picked=arr.filter(e=>ids.has(e.id)), rest=arr.filter(e=>!ids.has(e.id)); page().elements=[...rest,...picked]; }
  else if(dir==='back'){ const picked=arr.filter(e=>ids.has(e.id)), rest=arr.filter(e=>!ids.has(e.id)); page().elements=[...picked,...rest]; }
  else if(dir==='forward'){ for(let i=arr.length-2;i>=0;i--){ if(ids.has(arr[i].id)&&!ids.has(arr[i+1].id)) [arr[i],arr[i+1]]=[arr[i+1],arr[i]]; } }
  else if(dir==='backward'){ for(let i=1;i<arr.length;i++){ if(ids.has(arr[i].id)&&!ids.has(arr[i-1].id)) [arr[i],arr[i-1]]=[arr[i-1],arr[i]]; } }
  afterMutate();
}
// PPT식 우클릭 컨텍스트 메뉴 (일반 요소). 화살표 항목은 호버하면 옆으로 flyout이 펼쳐짐.
function showElementCtx(x,y,e){
  const m=document.getElementById('ctx-menu');
  document.getElementById('ctx-fly')?.remove();
  const fly=document.createElement('div'); fly.id='ctx-fly'; fly.className='ctx'; fly.style.cssText='position:fixed;display:none;z-index:9600'; document.body.appendChild(fly);
  const hasShape = selShapeEls().length>0 || e.type==='shape';
  const hasText  = selTextEls().length>0  || e.type==='text';
  const isIcon = e.type==='image' && !!e._iconSvg;
  const linkTargets=()=> (selAll().length?selAll():[e]);
  let flyTimer=null, curKey=null, curItem=null;
  const closeAll=()=>{ hideCtx(); };
  const BBTN='padding:6px 4px;background:var(--panel2);border:1px solid var(--border);border-radius:6px;color:var(--text);cursor:pointer;font-size:13px';
  // ── 메인 메뉴 ──
  const row=(ic,lb,kb,act)=>`<div class="ci" data-act="${act}" style="display:flex;align-items:center;gap:9px"><span style="width:18px;text-align:center">${ic}</span><span style="flex:1">${lb}</span>${kb?`<span style="color:var(--sub);font-size:11px;margin-left:18px">${kb}</span>`:''}</div>`;
  const fl =(ic,lb,key)=>`<div class="ci ci-fly" data-fly="${key}" style="display:flex;align-items:center;gap:9px"><span style="width:18px;text-align:center">${ic}</span><span style="flex:1">${lb}</span><span style="color:var(--sub)">▸</span></div>`;
  let h='';
  ...
  if(hasShape) h+=fl('▢','테두리','border');
  if(isIcon)   h+=fl('🔣','아이콘','icon');
  h+='<div class="sep"></div>';
  h+=fl('🔗','링크','link');
  ...
  function flyHTML(key){
    if(key==='layer') return [['z-front','⏫ 맨 앞으로'],['z-forward','🔼 앞으로'],['z-backward','🔽 뒤로'],['z-back','⏬ 맨 뒤로']].map(a=>`<div class="ci" data-act="${a[0]}">${a[1]}</div>`).join('');
    if(key==='align') return [['a-l','◧ 왼쪽'],['a-cx','▥ 가로 가운데'],['a-r','◨ 오른쪽'],['a-t','⊤ 위'],['a-cy','▤ 세로 가운데'],['a-b','⊥ 아래']].map(a=>`<div class="ci" data-act="${a[0]}">${a[1]}</div>`).join('');
    if(key==='shape') return [['rect','■ 사각형'],['rrect','▢ 둥근 사각형'],['circle','● 원/타원']].map(s=>`<div class="ci" data-shape="${s[0]}">${s[1]}</div>`).join('');
    if(key==='link'){ const others=project.pages.filter(p=>p.id!==page().id); let s=''; if(others.length) others.forEach(p=>{ s+=`<div class="ci ${e.link===p.id?'cur':''}" data-link="${p.id}">${e.link===p.id?'✓ ':''}${escapeHtml(p.name||'페이지')}</div>`; }); else s+='<div class="head">다른 페이지가 없어요</div>'; s+='<div class="sep"></div><div class="ci" data-act="link-new">＋ 새 페이지 연결</div>'; if(e.link) s+='<div class="ci" data-act="link-go">✎ 연결 페이지로</div><div class="ci" data-act="link-clear">✖ 링크 해제</div>'; return s; }
    if(key==='text'){
      const t=selTextEls()[0]||{}, wOn=(t.fontWeight||400)>=700, al=t.align||'center';
      const onS='background:var(--accent);color:#fff;border-color:var(--accent)';
      return `<div style="padding:7px;display:flex;flex-direction:column;gap:7px;min-width:218px">
        <select class="ct-font" style="width:100%;padding:6px 7px;background:var(--panel2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:12px">${_fontOpts(t.fontFamily)}</select>
        <div style="display:flex;gap:6px;align-items:center">
          <span style="font-size:11px;color:var(--sub)">크기</span><input class="ct-size" type="number" min="6" max="200" value="${t.fontSize||16}" style="width:58px;padding:6px;background:var(--panel2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:12px">
          <select class="ct-weight" style="flex:1;padding:6px;background:var(--panel2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:12px">${[['300','얇게'],['400','보통'],['500','중간'],['700','굵게'],['800','더굵게'],['900','매우굵게']].map(w=>`<option value="${w[0]}" ${String(t.fontWeight||400)===w[0]?'selected':''}>${w[1]}</option>`).join('')}</select>
        </div>
        <div style="display:flex;gap:5px">
          <button class="ct-tb" data-tb="bold" style="flex:1;font-weight:800;${BBTN};${wOn?onS:''}">B</button>
          <button class="ct-tb" data-tb="italic" style="flex:1;font-style:italic;${BBTN};${t.italic?onS:''}">I</button>
          <button class="ct-tb" data-tb="underline" style="flex:1;text-decoration:underline;${BBTN};${t.underline?onS:''}">U</button>
        </div>
        <div style="display:flex;gap:5px">
          <button class="ct-al" data-al="left" style="flex:1;${BBTN};${al==='left'?onS:''}">⬅</button>
          <button class="ct-al" data-al="center" style="flex:1;${BBTN};${al==='center'?onS:''}">↔</button>
          <button class="ct-al" data-al="right" style="flex:1;${BBTN};${al==='right'?onS:''}">➡</button>
          <button class="ct-al" data-al="justify" style="flex:1;${BBTN};${al==='justify'?onS:''}">▤</button>
        </div>
        <button class="ct-cp" data-cp="textColor" style="${BBTN};display:flex;align-items:center;gap:7px;justify-content:center"><span style="width:14px;height:14px;border-radius:3px;border:1px solid var(--border);background:${t.color||'#000'}"></span> 글자 색</button>
      </div>`;
    }
    if(key==='fill'){
      const s=selShapeEls()[0]||{};
      return `<div style="padding:7px;display:flex;flex-direction:column;gap:7px;min-width:178px">
        <button class="ct-cp" data-cp="fill" style="${BBTN};display:flex;align-items:center;gap:7px;justify-content:center"><span style="width:14px;height:14px;border-radius:3px;border:1px solid var(--border);background:${s.fill||'#ccc'}"></span> 채우기 색</button>
        <div style="display:flex;gap:6px;align-items:center"><span style="font-size:11px;color:var(--sub)">모서리</span><input class="ct-radius" type="number" min="0" max="400" value="${s.radius||0}" style="flex:1;padding:6px;background:var(--panel2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:12px"></div>
      </div>`;
    }
    if(key==='border'){
      const s=selShapeEls()[0]||{};
      return `<div style="padding:7px;display:flex;flex-direction:column;gap:7px;min-width:178px">
        <button class="ct-cp" data-cp="outline" style="${BBTN};display:flex;align-items:center;gap:7px;justify-content:center"><span style="width:14px;height:14px;border-radius:3px;border:1px solid var(--border);background:${s.borderColor||'#333'}"></span> 테두리 색</button>
        <div style="display:flex;gap:6px;align-items:center"><span style="font-size:11px;color:var(--sub)">두께</span><input class="ct-bw" type="number" min="0" max="40" value="${s.borderW||0}" style="flex:1;padding:6px;background:var(--panel2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:12px"></div>
      </div>`;
    }
    if(key==='icon'){
      const ic=e; const isMono=ic._iconMono!==false;
      const partCount=iconPartCount(ic._iconSvg||'');
      let s='<div style="padding:7px;display:flex;flex-direction:column;gap:7px;min-width:190px">';
      if(isMono) s+=`<button class="ct-cp" data-cp="iconElColor" style="${BBTN};display:flex;align-items:center;gap:7px;justify-content:center"><span style="width:14px;height:14px;border-radius:3px;border:1px solid var(--border);background:${ic._iconColor||'#333'}"></span> 아이콘 색</button>`;
      else s+=`<div style="font-size:11px;color:var(--sub)">여러 색 고정 아이콘 — 색 변경 불가</div>`;
      s+=`<div style="display:flex;gap:6px;align-items:center"><span style="font-size:11px;color:var(--sub);white-space:nowrap">굵기</span><input class="ct-icstroke" type="range" min="-3" max="6" step="0.25" value="${ic._iconThick!=null?ic._iconThick:0}" style="flex:1"></div>`;
      if(partCount>=2) s+=`<button class="ci" data-act="icon-parts" style="${BBTN}">🎨 부분 색칠 (칠해서 선택)</button>`;
      s+='</div>';
      return s;
    }
    return '';
  }
  function showFly(key,item){
    curKey=key; curItem=item; fly.innerHTML=flyHTML(key); fly.style.display='block';
    const r=item.getBoundingClientRect();
    let left=r.right+2; if(left+fly.offsetWidth>window.innerWidth-6) left=Math.max(6,r.left-fly.offsetWidth-2);
    let top=r.top; if(top+fly.offsetHeight>window.innerHeight-6) top=Math.max(6,window.innerHeight-fly.offsetHeight-6);
    fly.style.left=left+'px'; fly.style.top=top+'px';
    m.querySelectorAll('.ci-fly').forEach(it=>it.style.background=''); item.style.background='var(--accent)'; item.style.color='#fff';
    wireFly();
  }
  function refreshFly(){ if(curKey&&curItem) showFly(curKey,curItem); }
  function doAct(a){
    if(a==='cut') cutSel();
    ...
    else if(a==='link-go'){ const i=pageIndex(e.link); if(i>=0){ curPage=i; selId=null; renderCanvas(); renderPages(); renderProps(); toast('연결 페이지로 이동'); } }
    else if(a==='link-clear'){ linkTargets().forEach(x=>x.link=null); afterMutate(); toast('링크 해제됨'); }
    else if(a==='icon-parts'){ selId=e.id; openIconParts(); }
    else if(a==='icon-force-stroke'){ const ic=selId?el(selId):null; if(ic){ ic._iconStrokeForced=true; ic._iconStrokeable=true; if(ic._iconStroke==null) ic._iconStroke=2; refreshIconEl(ic); renderCanvas(); save(true); renderProps(); } }
    else if(a==='icon-unforce-stroke'){ const ic=selId?el(selId):null; if(ic){ ic._iconStrokeForced=false; ic._iconStrokeable=iconStrokeable(ic._iconSvg||''); if(!ic._iconStrokeable) ic._iconStroke=null; refreshIconEl(ic); renderCanvas(); save(true); renderProps(); } }
    if(a!=='copy') closeAll();
  }
  function wireFly(){
    fly.querySelectorAll('[data-act]').forEach(it=>it.addEventListener('click',()=>doAct(it.dataset.act)));
    fly.querySelectorAll('[data-shape]').forEach(it=>it.addEventListener('click',()=>{ selShapeEls().forEach(s=>s.shape=it.dataset.shape); afterMutate(); closeAll(); }));
    fly.querySelectorAll('[data-link]').forEach(it=>it.addEventListener('click',()=>{ linkTargets().forEach(z=>z.link=it.dataset.link); afterMutate(); toast('링크 연결됨 🔗'); closeAll(); }));
    fly.querySelectorAll('[data-cp]').forEach(b=>b.addEventListener('click',ev=>{ ev.stopPropagation(); toggleColorPopup(b.dataset.cp, b); }));
    const fs=fly.querySelector('.ct-font'); if(fs) fs.addEventListener('change',()=>{ const v=fs.value; loadFont(v); applyToTexts(t=>t.fontFamily=v); });
    const sz=fly.querySelector('.ct-size'); if(sz) sz.addEventListener('input',()=>{ const v=parseInt(sz.value)||16; applyToTexts(t=>t.fontSize=v,true); });
    const wt=fly.querySelector('.ct-weight'); if(wt) wt.addEventListener('change',()=>{ const v=parseInt(wt.value)||400; applyToTexts(t=>t.fontWeight=v); refreshFly(); });
    fly.querySelectorAll('.ct-tb').forEach(b=>b.addEventListener('click',()=>{ const k=b.dataset.tb, t0=selTextEls()[0]||{}; if(k==='bold'){ const on=(t0.fontWeight||400)>=700; applyToTexts(t=>t.fontWeight=on?400:700); } else if(k==='italic'){ const on=!t0.italic; applyToTexts(t=>t.italic=on); } else { const on=!t0.underline; applyToTexts(t=>t.underline=on); } refreshFly(); }));
    fly.querySelectorAll('.ct-al').forEach(b=>b.addEventListener('click',()=>{ applyToTexts(t=>t.align=b.dataset.al); refreshFly(); }));
    const rd=fly.querySelector('.ct-radius'); if(rd) rd.addEventListener('input',()=>{ const v=parseInt(rd.value)||0; applyToShapes(s=>{ if(s.shape!=='circle') s.radius=v; }); });
    const bw=fly.querySelector('.ct-bw'); if(bw) bw.addEventListener('input',()=>{ const v=parseInt(bw.value)||0; applyToShapes(s=>s.borderW=v); });
    if(isIcon && typeof CP_TARGETS!=='undefined' && !CP_TARGETS.iconElColor){
      CP_TARGETS.iconElColor={ label:'아이콘 색', rich:false,
        current:()=>{ const x=selId?el(selId):null; return (x&&x._iconColor)||'#333333'; },
        set:v=>{ const x=selId?el(selId):null; if(x&&x._iconSvg){ x._iconColor=(v==='transparent'?'#333333':v); refreshIconEl(x); renderCanvas(); save(true); } } };
    }
    const ics=fly.querySelector('.ct-icstroke'); if(ics) ics.addEventListener('input',()=>{ e._iconThick=parseFloat(ics.value); refreshIconEl(e); renderCanvas(); save(true); });
    ics && ics.addEventListener('change',()=>snapshot());
  }
  // ── 호버/클릭으로 flyout 열기 ──
  m.querySelectorAll('.ci-fly').forEach(it=>{
    it.addEventListener('mouseenter',()=>{ clearTimeout(flyTimer); showFly(it.dataset.fly, it); });
    it.addEventListener('mouseleave',()=>{ flyTimer=setTimeout(()=>fly.style.display='none',200); });
    it.addEventListener('click',ev=>{ ev.stopPropagation(); clearTimeout(flyTimer); showFly(it.dataset.fly, it); });
  });
  fly.addEventListener('mouseenter',()=>clearTimeout(flyTimer));
  fly.addEventListener('mouseleave',()=>{ flyTimer=setTimeout(()=>fly.style.display='none',200); });
  // ── 메인 메뉴 단순 액션 ──
  m.querySelectorAll('[data-act]').forEach(it=>it.addEventListener('click',()=>doAct(it.dataset.act)));
}
...
function showTableCtx(x,y,e,ev,rc){
  ...
  h+=`<div class="ct-vsep"></div>`;
  h+=`<button class="ct-btn" id="ct-border" title="테두리(두께·색·스타일·모서리)" style="width:auto;padding:0 4px;gap:1px">${_bdIcon('outer')}<span style="font-size:8px">▾</span></button>`;
  h+='</div>';
...
function showBorderDialog(e){
  ...
  card.innerHTML=`
    ...
    <div style="padding:4px 18px 0">
      <div style="${SL}">모서리 둥글기 — 표 전체</div>
      <div style="display:flex;align-items:center;gap:10px">
        <input type="range" id="bd-radius" min="0" max="40" value="${e.radius||0}" style="flex:1">
        <span id="bd-radius-v" style="font-size:12px;color:var(--text,#333);min-width:38px;text-align:right">${e.radius||0}px</span>
      </div>
    </div>
  ...
  // 모서리 둥글기 — 즉시 미리보기(표 본체에 바로 반영)
  $('#bd-radius').addEventListener('input',()=>{ e.radius=parseInt($('#bd-radius').value)||0; $('#bd-radius-v').textContent=e.radius+'px'; liveStyleEl(e); });
  ...
  $('#bd-ok').onclick=()=>{
    _tblApplyBorders(e,range,pend);
    // 둥근 모서리 프레임은 e.borderW/Color로 그려지므로, 바깥 테두리를 지정했으면 그 값을 e에 반영
    const outer=[pend.top,pend.right,pend.bottom,pend.left].find(v=>v&&v!=='none');
    if(outer){ e.borderW=outer.w; e.borderColor=outer.c; }
    liveStyleEl(e); snapshot(); close(); toast('테두리 적용됨');
  };
...
function hideCtx(){ document.getElementById('ctx-menu').style.display='none'; document.getElementById('ctx-fly')?.remove(); }
window.addEventListener('mousedown',e=>{ if(!e.target.closest('#ctx-menu') && !e.target.closest('#ctx-fly') && !e.target.closest('#fill-dd') && !e.target.closest('#tbl-bd-pop')) hideCtx(); });
...
function _collectUsedFonts(){
  ...
  (project.pages||[]).forEach(p=>(p.elements||[]).forEach(e=>{
    ...
    if(e.richFonts) e.richFonts.forEach(r=>{ if(r.font) s.add(r.font); });
    if(e.stRichFonts) e.stRichFonts.forEach(r=>{ if(r.font) s.add(r.font); });
  }));
...
document.getElementById('btn-ai').onclick=()=>{ document.getElementById('ai-modal').style.display='flex'; setTimeout(()=>{ const d=document.getElementById('ai-desc'); if(d) d.focus(); },50); };
...
document.getElementById('ai-img-folder').onclick=()=>document.getElementById('ai-img-file').click();
document.getElementById('ai-img-clear').onclick=e=>{ e.stopPropagation(); aiRefImgs=[]; renderAiThumbs(); };
// 참고 이미지 영역: 드래그&드롭으로도 첨부 (클릭으로는 파일창을 열지 않음)
(function(){ const z=document.getElementById('ai-img-pick'); if(!z) return;
  const hi=on=>{ z.style.borderColor = on?'var(--accent2)':''; };
  z.addEventListener('dragover',e=>{ e.preventDefault(); hi(true); });
  z.addEventListener('dragleave',()=>hi(false));
  z.addEventListener('drop',async e=>{ e.preventDefault(); hi(false);
    const fs=[...((e.dataTransfer&&e.dataTransfer.files)||[])].filter(f=>f.type.indexOf('image')===0);
    for(const f of fs) await addAiImg(f);
  });
})();
...
document.getElementById('ai-url-go').onclick=async()=>{
  ...
  try{
    ...
    const d=res.data||{};
    aiSiteRef=d;
    // 렌더된 스크린샷이 오면 참고 이미지로 자동 첨부 → AI가 실제 화면을 봄
    let shot=false;
    if(d.screenshot){ const mm=String(d.screenshot).match(/^data:([^;]+);base64,(.+)$/); if(mm){ aiRefImgs.push({mediaType:mm[1], data:mm[2]}); renderAiThumbs(); shot=true; } }
    const cnt=(d.h1?.length||0)+(d.h2?.length||0)+(d.h3?.length||0);
    st.style.color='var(--accent2)';
    st.textContent=`✓ 분석 완료: "${d.title||url}" · 제목 ${cnt}개 · 색상 ${(d.colors||[]).length}개${(d.effects&&d.effects.length)?` · 효과 ${d.effects.join(', ')}`:''}${shot?' · 📷 스크린샷 첨부됨':''} — '초안 생성하기'를 누르면 이 구조를 참고합니다`;
  }catch(e){
    aiSiteRef=null;
...
document.getElementById('ai-go').onclick=async()=>{
  ...
  const typeLabel=appendMode?'섹션(한 묶음)':'병원 페이지';
  const targetPageId=page().id; // 생성 시작 시점의 페이지 고정 — 완료 시 다른 페이지로 이동해도 여기에만 적용
  const SCHEMA_EXAMPLE=`{"name":"홈","bg":"#ffffff","elements":[
  {"type":"shape","x":0,"y":0,"w":${PAGE_W},"h":520,"shape":"rect","fill":"#1a2b5c","bleed":true},
  ...
  {"type":"shape","x":120,"y":410,"w":200,"h":60,"shape":"rrect","fill":"#3b82f6","radius":30,"stext":"진료 예약하기","stColor":"#ffffff","stSize":18,"stWeight":700,"stAlign":"center","stValign":"middle"},
  ...
  {"type":"tabsection","x":120,"y":720,"w":960,"tabs":[
    {"label":"진료안내","body":[{"type":"text","x":40,"y":40,"w":880,"h":40,"text":"내과 · 외과 · 정형외과","fontSize":24,"fontWeight":700,"color":"#1a2b5c"},{"type":"text","x":40,"y":100,"w":880,"h":120,"text":"각 분야 전문의가 정밀하게 진료합니다.","fontSize":17,"color":"#555"}]},
    {"label":"의료진","body":[{"type":"text","x":40,"y":40,"w":880,"h":40,"text":"신뢰할 수 있는 의료진","fontSize":24,"fontWeight":700,"color":"#1a2b5c"}]},
    {"label":"오시는길","body":[{"type":"text","x":40,"y":40,"w":880,"h":40,"text":"서울시 ○○구 ○○로 123","fontSize":20,"fontWeight":700,"color":"#1a2b5c"}]}
  ]}
]}`;
...
- shape: {"type":"shape","x","y","w","h","shape":"rect|rrect|circle","fill":"#hex","radius":숫자} ← 섹션 배경·카드 배경에 사용. ★폭 전체를 덮는 섹션 배경 띠(예: x:0,w:${PAGE_W})에는 "bleed":true 를 더해라 → 발행 시 화면 좌우 끝까지 꽉 참(여백 없음).
- shape(글자 포함): 위 shape에 "stext":"버튼/라벨 글자","stColor":"#hex","stSize":px,"stWeight":700,"stAlign":"center","stValign":"middle"를 더한 형태. ★버튼·태그·통계숫자·아이콘칩처럼 '도형+짧은 글자'는 반드시 이 stext 도형 하나로 만들어라. 도형 위에 별도 text를 겹치지 마라(요소 하나로).
- image: {"type":"image","x","y","w","h"} ← src는 넣지 마라(자동으로 사진 자리표시자가 들어간다). 화면 전체를 덮는 히어로 배경 사진이면 "bleed":true 를 더해 화면 끝까지 차게 하라.
...
- tabsection(탭 섹션): {"type":"tabsection","x","y","w","tabs":[{"label":"탭이름","body":[ ...패널 기준(좌상단 0,0) 상대좌표의 text/shape/image/table 요소들 ]}, ...]} ← 한 자리에서 탭을 눌러 내용이 바뀌는 섹션(예: 진료안내/의료진/오시는길). 탭 2~6개. body 각 요소의 x·y는 패널 영역(가로 0~w, 세로 0부터) 기준 상대좌표. ★탭 버튼·선택표시·전환 배선은 시스템이 자동 생성하니 너는 label과 body 내용만 채워라. 절대 탭 버튼을 shape로 직접 만들거나 fx에 tab-trigger/tab-content를 직접 쓰지 마라 — 탭이 필요하면 무조건 이 tabsection 형식만 사용.
규칙:
- 색상(bg, fill, color)은 참고 자료의 톤에 맞춰라.
- 문구는 참고 자료에 있으면 반영하고, 없으면 병원에 어울리는 **자연스러운 한국어 문구를 직접 작성**하라. "제목","본문" 같은 자리표시자는 쓰지 마라.
- 버튼·태그·통계 숫자 등 '도형+짧은 글자'는 stext 도형 하나로(별도 text 겹치기 금지). 카드는 연한 배경 shape 위에 제목/본문 text를 얹어 구성.
- fontFamily는 보통 생략(기본 본고딕). 강조 제목엔 "Black Han Sans","Do Hyeon" 등을 써도 된다.
- **섹션 배경은 화면 끝까지(bleed), 내용은 가운데**: 색이 다른 섹션마다 폭 전체 배경 띠 shape(x:0,w:${PAGE_W},"bleed":true)를 깔고, 그 위 제목·본문·카드 등 내용은 ${PAGE_W} 폭 안에 배치(내용 요소엔 bleed 금지). 이러면 넓은 화면에서 배경은 끝까지 차고 글자는 중앙에 모인다.
- **맨 아래에 푸터 섹션**을 넣어라: 어두운 배경 띠(폭 전체 shape, "bleed":true) 위에 병원명·주소·전화·진료시간·저작권(© 2026 …)을 흰/연한 글자로.
- **주요 요소에 효과를 절제해서 "fx"로 넣어도 된다**(없어도 됨): 큰 제목 {"type":"char-reveal"}, 히어로/큰 사진 {"type":"mask-wipe"}·{"type":"blur-in"}·{"type":"parallax","speed":0.15}, 같은 줄 카드들 {"type":"scroll-reveal","dir":"up","delay":0/100/200}(스태거), 주목 버튼 1개 {"type":"pulse"} 또는 {"type":"hover-lift"}, 통계 숫자 도형 {"type":"counter","from":0,"to":100,"suffix":"+"}, 사진 여러 장은 image에 {"type":"slider","peek":true}(양옆 살짝 보임). 과하지 않게, 본문·작은 글자엔 금지.
...
${SCHEMA_EXAMPLE}`;
  ...
  if(aiSiteRef){
    const s=aiSiteRef;
    const brand=(s.brandColors&&s.brandColors.length)?`\n- 브랜드 색(CSS변수에서 추출 — 가장 우선 고려): ${s.brandColors.join(', ')}`:'';
    const effLine=(s.effects&&s.effects.length)?`\n- 원본이 쓰는 효과(가능하면 비슷한 fx로 반영하라): ${s.effects.join(', ')}`:'';
    userText = `아래 [참고 사이트]를 참고해 '${typeLabel}'를 처음부터 만들어줘. 똑같이 베끼지 말고 아래에서 지정한 항목만 반영해 재구성하라.\n`+
      `[참고 사이트]\n- 사이트명: ${s.title||''}\n- 설명: ${s.description||''}\n- 대표 색상: ${(s.colors||[]).join(', ')||s.themeColor||''}${brand}\n- 큰제목: ${(s.h1||[]).join(' / ')}\n- 중제목: ${(s.h2||[]).slice(0,8).join(' / ')}\n- 소제목: ${(s.h3||[]).slice(0,8).join(' / ')}\n- 버튼 문구: ${(s.buttons||[]).join(' / ')}${effLine}\n\n추가 요청: ${desc||'(없음)'}`;
  }
  ...
  try{
    ...
    let spec=parseAiJson(text);
    if(!spec) throw new Error('AI 응답을 JSON으로 해석하지 못했습니다. 다시 시도해 주세요.');
    // ── A. 자기검증 루프(선택) — 1차 결과를 AI가 스스로 검수해 개선본으로 교체 ──
    if(document.getElementById('ai-refine')?.checked){
      try{
        aiStatus('AI가 생성 결과를 스스로 점검·개선 중입니다… (품질 향상, 10~30초)');
        const reviewSys = sysFull + `\n\n────────\n★지금은 "냉정한 디자인 검수자" 역할이다. 아래 [초안 JSON]을 위 모든 규칙·스키마를 기준으로 점검하고, 발견한 문제를 고친 **전체 JSON**을 출력하라.\n점검 항목: 요소 겹침/잘림, 글자 위계(제목>소제목>본문) 불일치, 색 대비 부족(배경 위 글자 가독성), 빈 문구·"제목/본문" 같은 자리표시자, 푸터 누락, 섹션 간 간격 불균형, 캔버스(0~${PAGE_W}) 밖 좌표, 표 cells 누락, 버튼이 stext 도형 하나가 아닌 도형+text로 분리돼 있으면 stext 도형 하나로 합칠 것. ★tabsection·slider·counter 등 컴포넌트/효과는 type·구조·배선을 절대 바꾸지 말고(탭 버튼을 직접 만들지 말 것) 내부 label·body 문구·색·정렬만 다듬어라. 요소의 "bleed":true(섹션 배경 풀블리드)·fx 속성은 그대로 보존하라. 초안이 이미 충분히 좋으면 과한 변경 없이 그대로 둬도 된다. 설명·마크다운 없이 오직 개선된 JSON 하나만, 동일 스키마로 끝까지 유효하게 출력한다.`;
        const reviewUser = `[원래 요청]\n${typeof userText==='string'?userText:'(이미지 첨부 기반 요청)'}\n\n[초안 JSON]\n${JSON.stringify(spec)}`;
        const r2=await aiProxy({ body:{ model:'claude-opus-4-8', max_tokens:12000, system:reviewSys, messages:[{role:'user',content:reviewUser}] }});
        const spec2=parseAiJson(r2.data?.content?.[0]?.text||'');
        if(spec2 && Array.isArray(spec2.elements) && spec2.elements.length) spec=spec2;
      }catch(_){ /* 검수 실패 시 1차 초안을 그대로 사용 */ }
    }
    // 생성 시작 시 고정한 페이지로 적용 (그 사이 다른 페이지로 이동했어도 그 페이지엔 영향 없음)
    const _ti=project.pages.findIndex(pp=>pp.id===targetPageId);
    if(_ti<0){ aiStatus('대상 페이지가 삭제되어 적용을 취소했습니다.'); return; }
    if(curPage!==_ti){ curPage=_ti; renderPages(); }
    const p=page();
    const norm=e=>{
    ...
    const rawEls=expandAiComponents(spec.elements);   // tabsection 등 고수준 노드 → 정확한 저수준 요소
    const newEls=rawEls.filter(e=>e&&e.type).map(norm);
    if(appendMode){
...
// ── B. 피드백 학습 — 사용자가 저장한 템플릿(긍정 신호)에서 취향을 추출해 프롬프트로 주입 ──
function deriveAiPrefs(){
  let tpls; try{ tpls=getTemplates(); }catch(_){ return ''; }
  if(!tpls||!tpls.length) return '';
  const recent=tpls.slice(-12);                 // 최근 저장 우선(최대 12개)
  const titleSizes=[],bodySizes=[],colorCount={},famCount={}; let footer=0,tables=0; const n=recent.length;
  let stextBtn=0,plainBtn=0,tabUse=0,sliderUse=0,fxUse=0; const fxKinds={};
  for(const t of recent){
    const els=(t.page&&t.page.elements)||[]; let hasFooter=false,hasTable=false,hasTab=false,hasSlider=false;
    for(const e of els){
      if(e.type==='text'){
        if(e.fontSize>=26) titleSizes.push(e.fontSize); else if(e.fontSize&&e.fontSize<=18) bodySizes.push(e.fontSize);
        if(e.color){ const c=e.color.toLowerCase(); colorCount[c]=(colorCount[c]||0)+1; }
        if(e.fontFamily&&e.fontFamily!=='Noto Sans KR') famCount[e.fontFamily]=(famCount[e.fontFamily]||0)+1;
        if(/©|저작권|all rights/i.test(e.text||'')) hasFooter=true;
        // 작은 글자 도형 위에 겹친 '분리형 버튼' 추정
        if((e.fontSize||0)<=22 && /예약|상담|문의|신청|바로가기|자세히|더보기|button|click/i.test(e.text||'')) plainBtn++;
      } else if(e.type==='shape'){
        if(e.fill){ const c=e.fill.toLowerCase(); colorCount[c]=(colorCount[c]||0)+1; }
        if(e.stext && e.stext.length<=14) stextBtn++;       // 글자 들어간 도형(=요소 하나 버튼/칩)
      }
      else if(e.type==='table') hasTable=true;
      const ft=e.fx&&e.fx.type;
      if(ft){ fxUse++; fxKinds[ft]=(fxKinds[ft]||0)+1;
        if(ft==='tab-trigger') hasTab=true; if(ft==='slider') hasSlider=true; }
    }
    if(hasFooter) footer++; if(hasTable) tables++; if(hasTab) tabUse++; if(hasSlider) sliderUse++;
  }
  const med=a=>{ if(!a.length) return 0; const s=[...a].sort((x,y)=>x-y); return s[Math.floor(s.length/2)]; };
  const topColors=Object.entries(colorCount).sort((a,b)=>b[1]-a[1]).slice(0,5).map(c=>c[0]);
  const topFam=(Object.entries(famCount).sort((a,b)=>b[1]-a[1])[0]||[])[0];
  const topFx=Object.entries(fxKinds).filter(([k])=>!/^tab-/.test(k)).sort((a,b)=>b[1]-a[1]).slice(0,4).map(c=>c[0]);
  const ts=med(titleSizes),bs=med(bodySizes), lines=[];
  if(topColors.length) lines.push(`자주 쓰는 색: ${topColors.join(', ')} — 참고자료 색과 충돌하지 않으면 이 톤을 우선 고려`);
  if(ts) lines.push(`제목 글자 크기는 ${ts}px 안팎을 선호`);
  if(bs) lines.push(`본문 글자 크기는 ${bs}px 안팎을 선호`);
  if(topFam) lines.push(`강조 제목 폰트로 "${topFam}"를 즐겨 씀`);
  if(stextBtn>=plainBtn) lines.push('버튼·태그·칩은 글자 들어간 도형 하나(stext)로 만드는 방식을 선호 — 도형+텍스트로 분리하지 말 것');
  if(tabUse/n>=0.3) lines.push('탭 섹션(tabsection)으로 정보를 묶는 구성을 즐겨 씀 — 어울리면 1개 포함');
  if(sliderUse/n>=0.3) lines.push('사진 여러 장은 슬라이더(slider, peek)로 보여주는 방식을 선호');
  if(footer/n>=0.5) lines.push('하단 푸터 섹션을 선호');
  if(tables/n>=0.4) lines.push('정보 정리에 표(table)를 즐겨 씀');
  if(topFx.length) lines.push(`즐겨 쓰는 효과(fx): ${topFx.join(', ')} — 비슷한 톤으로 절제해서 적용`);
  if(!lines.length) return '';
  return `\n\n[사용자 선호 — 과거에 저장한 템플릿 ${n}개에서 학습한 취향. 참고자료·요청과 충돌하지 않는 선에서 자연스럽게 반영하라]\n- ${lines.join('\n- ')}`;
}
...
function applyAiActions(actions){
  ...
  for(const a of actions){
    ...
    else if(a.type==='add_tabsection'){
      // 탭 섹션 생성 — 저수준 배선은 buildTabSectionEls가 정확히 만든다(AI가 직접 배선하지 않음)
      const o=a.section||a;
      try{ const { els }=buildTabSectionEls({ x:o.x, y:o.y, w:o.w, btnH:o.btnH, panelH:o.panelH, activeColor:o.activeColor||o.color, idleColor:o.idleColor, tabs:o.tabs });
        page().elements.push(...els); selId=null; selIds=new Set(els.filter(e=>e.fx&&e.fx.type==='tab-trigger').map(e=>e.id)); changed=true;
      }catch(_){}
    }
...
10. {"type":"add_tabsection","section":{"x","y","w","tabs":[{"label":"탭이름","body":[패널 기준(0,0) 상대좌표 text/shape/image 요소들]},...]}} ← 탭 섹션(한 자리에서 탭 눌러 내용 전환). 탭 2~6개. ★탭 버튼·전환 배선은 자동 생성되니 label·body만 채워라. 절대 tab-trigger/tab-content fx나 탭 버튼 shape를 직접 만들지 마라 — 탭이 필요하면 무조건 이 액션 사용.
...
반드시 JSON만 응답: {"reply":"한국어설명","actions":[...]}`+deviceHint();
  ...
  try{
    const res=await aiProxy({body:{model:'claude-sonnet-4-6',max_tokens:16000,system:sys,messages:_chatHistory.slice(-8)}});
...
document.addEventListener('paste', ev=>{
  ...
  // AI 초안 모달이 열려 있으면 포커스 위치와 무관하게 '참고 이미지'로 첨부 (단일 핸들러 — 중복/캔버스 누출 방지)
  const aiM=document.getElementById('ai-modal');
  if(aiM && getComputedStyle(aiM).display!=='none'){
    const fi=clipboardImageFile(ev);
    if(fi){ ev.preventDefault(); addAiImg(fi); toast('스크린샷 첨부됨 📋'); }
    return; // 이미지 아니면(텍스트 등) 기본 붙여넣기 그대로
  }
  if(ae&&ae.closest&&ae.closest('#ai-chat-panel,#login-modal,#find-modal,#cloud-modal')) return;
...
// 쌍둥이 미러링 — 같은 자리의 다른 상태(예: 선택 전↔선택 후 버튼, 탭별 내용 패널)는 e.twin을 공유.
// 전체 보기에서 한쪽을 이동/크기조절하면 숨은 짝도 함께 따라가게 한다(탭별 편집 중엔 미러링 안 함 → 개별).
function twinMates(idSet){
  // 전체모드: 절대규칙 — 하나를 집든 여러 개를 집든 항상 숨은 짝까지 함께 이동·크기.
  // 탭별 편집 중에만 개별 조절(선택한 상태만 따로) → 그 위치가 그대로 유지됨.
  if(_tabEditView) return [];
  const twins=new Set();
  idSet.forEach(id=>{ const e=el(id); if(e&&e.twin) twins.add(e.twin); });
  if(!twins.size) return [];
  return page().elements.filter(e=>e.twin && twins.has(e.twin) && !idSet.has(e.id));
}
// 삭제 확장 — 탭 요소를 지우면 같은 탭(같은 group+idx)의 숨은 짝(선택 후 버튼·내용 등)까지 함께 삭제
function expandTabDelete(idSet){
  const slots=new Set();
  idSet.forEach(id=>{ const e=el(id), f=e&&e.fx; if(f&&(f.type==='tab-trigger'||f.type==='tab-content')) slots.add(f.group+'|'+(f.idx||0)); });
  if(!slots.size) return idSet;
  const out=new Set(idSet);
  page().elements.forEach(e=>{ const f=e.fx; if(f&&(f.type==='tab-trigger'||f.type==='tab-content')&&slots.has(f.group+'|'+(f.idx||0))) out.add(e.id); });
  return out;
}
...
function arrDel(){ if(!selIds.size)return; const del=expandTabDelete(selIds); page().elements=page().elements.filter(x=>!del.has(x.id)); selId=null; selIds=new Set(); afterMutate(); }
...
function setupHomeFontPicker(){
  ...
  function render(q=''){
    ...
    list.querySelectorAll('.fitem').forEach(it=>{
      it.addEventListener('mousedown', ev=>{ if(ev.target.dataset.delfont) return; if(_activeRichSelection()) ev.preventDefault(); }); // 편집 중 선택 유지
      it.addEventListener('click',ev=>{
        if(ev.target.dataset.delfont) return;
        const fam=it.dataset.fam; loadFont(fam);
        if(applyFontToSelection(fam)){ toast(`선택한 글자에 "${fam}" 폰트 적용됨`); closeAllDD(); return; }
  ...
  btn.addEventListener('mousedown', e=>{ if(_activeRichSelection()) e.preventDefault(); });
  btn.addEventListener('click',e=>{ if(dd.style.display==='block'){ closeAllDD(); return; } const hadSel=!!_activeRichSelection(); render(); openDD(dd,e.currentTarget); if(!hadSel) setTimeout(()=>search.focus(),0); });
  ...
  document.getElementById('rb-font-file-btn').addEventListener('click',()=>{
    fileInput.onchange=()=>{ const fs=fileInput.files; fileInput.value=''; if(fs&&fs.length) importFontFiles(fs,()=>{ render(); const fam=(selTextEls()[0]||{}).fontFamily; if(fam) setHomeFontLabel(fam); }); };
    fileInput.click();
  });
}
...
const CP_TARGETS={
  ...
  hmSubColor:{ label:'하위 메뉴 글자색', rich:false, current:()=>hamburgerCfg().subColor||hamburgerCfg().color||'#1a2b5c',
    set:v=>{ hamburgerCfg().subColor=v; save(true); refreshHambMenu(); } },
...
document.getElementById('rb2-tabsection')?.addEventListener('click',makeTabSection);
// 보기 탭 버튼
